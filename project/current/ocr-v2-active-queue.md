...
> Stage 2 step 3 done: `main.requestOcrRun` + `settings._dispatchOcrAction` (rebuild/redo) → shared `client.runAction` (`ocr.run`/`ocr.rebuild_derived`/`ocr.redo`); Stop → `cancelActiveOperation`; credential fail-closed via action-registry availability. `ocr-process-controller.ts` + its test tombstoned. Stage 2 complete pending owner verification.
