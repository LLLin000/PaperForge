...
- P acceptance is the sole authority action that may create a reviewed canonical figure. It requires the live reviewed-plan snapshot, exact staged paths, plan-owned member refs, unique canonical identity/asset ownership, and valid staged image/Markdown. It uses the same paper lock, schema-v2 journal, destination CAS, atomic writes, rollback-before-commit, committed-cleanup recovery, provenance merge, and post-audit/report-refresh boundary as R. It may update only the accepted figure's canonical inventory row, JPG, Markdown, materialization provenance, and proposal report; it never changes OCR blocks, source PDFs, asset indexes, `meta.json`, or user-authored notes.
...
- P remains a human-authority action, not an automatic consequence of a dry-run. Bounded-slot anchors derive from unique, non-conflicted `main` rows; acceptance may create the canonical object only from the reviewed plan and only after its live authority checks pass.
