...
- A committed promotion never rolls back because the final report refresh fails. `state=committed` recovery does not require backups, best-effort cleans the transaction directory, and removes the journal while preserving production. A report that writes successfully but returns audit `state=FAILED` is surfaced as `report_refresh=written` plus `report_audit_state=FAILED`; refresh exceptions/non-object results use `report_refresh=failed`.
