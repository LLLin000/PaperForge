...
脚本会自动检查 Python、安装 PaperForge，然后提示运行 `paperforge setup --headless`。
...
│   ├── [literature_dir]/          # 正式文献笔记（含 frontmatter 状态跟踪）
│   │   └── [domain]/              # 按 Zotero 分类
│   │       └── [key] - [Title].md # 正式文献笔记
│
...
这个命令会读取 BBT JSON 导出，直接生成正式文献笔记 `Literature/[domain]/[key] - [Title].md`，并创建 Obsidian Base 视图。
...
| `paperforge setup --headless` | 重新运行安装向导（headless 模式） | 修改配置时使用 |
...
| control_dir | `LiteratureControl` | 状态控制（v1.9+ 已废弃，状态移至 formal note） |
