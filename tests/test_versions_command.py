...
def test_backups_recognizes_legacy_filenames_and_timestamps(tmp_path: Path) -> None:
    vault = tmp_path / "v"
    vault.mkdir()
    root = _seed(vault)
    ...
    b = backups[0]
    assert b["label"] == "backup-20250102030405"
    assert b["created_at"] == "2025-01-02T03:04:05Z"
    assert b["fulltext_size"] == (
        root / "backups" / "fulltext.pre-rebuild.20250102030405.md"
    ).stat().st_size
    assert b["path"].replace("\\", "/").endswith(
        "fulltext.pre-rebuild.20250102030405.md"
    )
...
def test_paths_constructs_canonical_artifact_paths(tmp_path: Path) -> None:
    ...
    rc3, payload3, raw3 = _run(vault, "paths", key=KEY, label="backup-20250102030405")
    assert rc3 == 0, raw3
