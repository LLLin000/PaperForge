...
export class PaperForgeSettingTab extends PluginSettingTab {
  ...
  private _buildProgress: { current: number; total: number; key: string } = { current: 0, total: 0, key: "" };
    ...
    const renderEmbedUI = () => {
      ...
      if (!this.plugin._embedProcess && buildState.status === "running") {
        this.plugin._embedProgress = {
          ...
          total:
            typeof buildState.total === "number" ? buildState.total : 1,
          key:
            typeof buildState.paper_id === "string"
              ? buildState.paper_id
              : "",
        };
      }
      ...
      const objectChunkCount =
        typeof vr?.object_chunk_count === "number"
          ? vr.object_chunk_count
          : 0;
      const chunkCount =
      ...
      if (health && typeof health.summary === "object" && health.summary !== null && "status" in health.summary) {
