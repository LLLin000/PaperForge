import { ItemView, WorkspaceLeaf, Notice, MarkdownRenderer, App, TFile } from "obsidian";
import * as fs from "fs";
import * as path from "path";
import { execFile, spawn, execFileSync } from "child_process";
import {
  VIEW_TYPE_PAPERFORGE, ACTIONS, ActionDef, PaperForgeSettings,
  PF_ICON_ID, overlayEntryWorkflowState, patchEntryWorkflowState
} from "../constants";
import { t } from "../i18n";
import {
  resolveVaultPaths, getMemoryRuntime, getVectorRuntime, getRuntimeHealth,
  isMemoryReady, isVectorReady, isHealthOk, getMemoryStatusText, getVectorStatusText,
  getCachedPython, buildSnapshot, shouldRenderVectorReady
} from "../services/memory-state";
import {
  resolvePythonExecutable, buildCommandArgs, runSubprocess, classifyError,
...
export class PaperForgeStatusView extends ItemView {
  ...
  _fallbackFetchStats(quiet: boolean, vp: string, plugin: any) {
    ...
    try {
      ...
      const healthCounts: Record<string, { healthy: number; unhealthy: number }> = {
      ...
      const { path: pythonExe, extraArgs = [] } = resolvePythonExecutable(vp, plugin?.settings ?? null, undefined, undefined);
      (execFile as any)(pythonExe, [...extraArgs, '-m', 'paperforge', 'status', '--json'], { cwd: vp, timeout: 30000 }, (err2: any, stdout: string) => {
        if (err2) {
  ...
  _findEntry(key: string): any {
    if (!key) return null;
    const entry = this._getCachedIndex().find((item: any) => item.zotero_key === key) || null;
    return overlayEntryWorkflowState(this.app, entry);
  }

  _patchCachedEntry(key: string, patch: any) {
    if (!key || !this._cachedItems) return;
    const idx = this._cachedItems.findIndex((item: any) => item.zotero_key === key);
    if (idx === -1) return;
  ...
  _renderStats(d: any) {
    if (this._versionBadge) this._versionBadge.setText(this._paperforgeVersion || (d.version ? 'v' + d.version : 'v\u2014'));
  ...
  _renderLifecycleStepper(container: HTMLElement, lifecycle: any, currentStage: string) {
  ...
  _renderMaturityGauge(container: HTMLElement, maturityLevel: number | null, blockingChecks: any) {
  ...
  _renderBarChart(container: HTMLElement, lifecycleCounts: Record<string, number>) {
  ...
  _resolveModeForFile(file: any): { mode: 'global' | 'paper' | 'collection'; filePath: string | null; key: string | null; domain: string | null } {
  ...
  _renderGlobalMode() {
    ...
    if (!pyVer) {
      try {
        const vp = (this.app.vault.adapter as any).basePath as string;
        const { path: pyExe, extraArgs = [] } = resolvePythonExecutable(vp, plugin?.settings ?? null, undefined, undefined);
    ...
    try {
      ...
      if (fs.existsSync(exportsDir)) {
        const files = fs.readdirSync(exportsDir).filter((f: string) => f.endsWith('.json'));
    ...
    memDetail = (rh && (rh.summary as any)?.reason) || (rh && (rh.summary as any)?.status) || 'Unknown';
    ...
    hubBtn.addEventListener('click', () => {
      ...
      if (baseFolder) {
        ...
        if ((baseFolder as any).children) {
          baseFile = (baseFolder as any).children.find((f: any) => f.extension === 'base');
        }
        if (baseFile) {
  ...
  _renderSystemStatusRow(container: HTMLElement, label: string, status: string, detail: string) {
  ...
  _renderPaperOverviewCard(container: HTMLElement, entry: any) {
    ...
    if (entry.note_path) {
      ...
      if (noteFile) {
        this.app.vault.read(noteFile as TFile).then((content: string) => {
  ...
  _renderRecentDiscussionCard(container: HTMLElement, entry: any) {
    ...
    (this.app.vault.adapter as any).exists(mdPath).then((exists: boolean) => {
      if (!exists) return;
  ...
  _parseDiscussionMD(content: string): { question: string; answer: string }[] | null {
    ...
    for (const block of qaBlocks) {
      ...
      const answer = block.substring(answerMatch.index! + '\u89E3\u7B54:'.length + 4).trim();
  ...
  _renderPaperTechnicalDetails(container: HTMLElement, entry: any) {
    ...
    for (const tf of toggleFields) {
      ...
      cb.addEventListener('change', async () => {
        ...
        await this.app.fileManager.processFrontMatter(noteFile as TFile, (fm: any) => { fm[tf.key] = newVal; });
  ...
  _renderNextStepCard(container: HTMLElement, entry: any, key: string) {
    ...
    const stepInfo: Record<string, { label: string; text: string; cmd: string | null; icon: string }> = {
  ...
  static async open(plugin: IPluginRef) {
    ...
    if (leaf) {
      await leaf.setViewState({ type: VIEW_TYPE_PAPERFORGE, active: true } as any);
