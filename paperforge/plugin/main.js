...
var me = class {
  constructor(c) {
    ...
    if (
      ...
      ((this.runtimeDir = c.runtimeDir),
        (this.rootDir = L.dirname(c.runtimeDir)),
        (this.pluginVersion =
      ...
      let h = ar.homedir();
      ((this.rootDir = L.join(h, ".paperforge", "runtime")),
        (this.runtimeDir = L.join(this.rootDir, nr(e, t))),
        (this.pluginVersion =
  ...
  async ensure(c) {
    ...
    if (this._currentSlotExists(e) && !t) {
      ...
      try {
        let v = this._fs.readFileSync(this.pointerPath, "utf-8"),
          k = JSON.parse(v),
          m = typeof k.version == "string" ? k.version : null;
        y = m !== null && m !== e;
      } catch (v) {}
      if (y) {
        let v = L.join(this.runtimeDir, `v${e}`),
          k = L.join(v, "venv"),
          m =
            this.osPlatform === "win32"
              ? L.join(k, "Scripts", "python.exe")
              : L.join(k, "bin", "python");
        try {
        ...
        let P = L.relative(L.dirname(this.pointerPath), m),
          B = JSON.stringify(
            {
              schema_version: 1,
              version: e,
        ...
        (this._fs.writeFileSync(I, B, "utf-8"),
    ...
    let i = t
      ...
      o =
        this.osPlatform === "win32"
          ? L.join(s, "Scripts", "python.exe")
          : L.join(s, "bin", "python");
    try {
      this._fs.mkdirSync(i, { recursive: !0 });
      let { promise: y, reject: v, resolve: k } = Ye();
      (this._execFile(
        n.path,
        ["-m", "venv", s],
      ...
      return this._slotFailed(e, "VENV_CREATION_FAILED", y, i);
    }
    if (r != null && r.aborted) return this._abortedHealth();
    try {
      let { promise: y, reject: v, resolve: k } = Ye();
      ...
      return this._slotFailed(e, "PIP_INSTALL_FAILED", y, i);
    }
    if (r != null && r.aborted) return this._abortedHealth();
    try {
      let { promise: y, reject: v, resolve: k } = Ye();
      ...
      return this._slotFailed(e, "VERIFY_FAILED", y, i);
    }
    let l = null,
      p = null;
    try {
      let y = this._fs.readFileSync(this.pointerPath, "utf-8"),
        v = JSON.parse(y);
    ...
    let f = L.relative(L.dirname(this.pointerPath), o),
      _ = JSON.stringify(
        {
          schema_version: 1,
          version: e,
  ...
  _currentSlotExists(c) {
    let e = L.join(this.runtimeDir, `v${c}`);
...
`
    ...
    let n = (_, g = {}) =>
        new Promise((h, b) => {
          let { path: E, args: y = [] } = this._resolvePython(),
            v = (0, Te.spawn)(E, [...y, ..._], {
          ...
          (v.stdout.on("data", (w) => {
            ...
            ((k += x), g.logStdout && this._processSetupOutput(x));
          }),
            v.stderr.on("data", (w) => {
              let x = w.toString("utf-8");
              ((m += x), this._log("[stderr] " + x.trim()));
            }),
            v.on("close", (w) => {
              w === 0
                ? h({ stdout: k, stderr: m })
                : b(new Error(m.trim() || k.trim() || `exit code ${w}`));
            }),
            v.on("error", (w) => b(w)));
        }),
      i = [
      ...
      f
        ...
        .addEventListener("click", () => {
          let h = encodeURIComponent($e(this._titleInput.value).clean),
            ...
            y = `${this._githubUrl}?title=${h}&body=${b}&labels=${E}`;
...
var Oe = class Oe extends R.PluginSettingTab {
  ...
  _renderInstallationDetail(e) {
    ...
    let n = r.createDiv({ cls: "pf-config" }),
      i = (m, w, x, S) => {
        ...
        (P.createEl("span", { cls: "pf-config-key", text: m }),
          P.createEl("span", { cls: S, text: w }),
    ...
    (y(`"${s}" --version`, { timeout: 5e3 }, (m) => {
      let w = n.children[1];
      if (w) {
        let x = w.querySelector(".pf-status-checking");
        x &&
          ((x.textContent = m ? "\u2717" : "\u2713"),
          (x.className = m ? "pf-status-error" : "pf-status-ok"));
      }
    }),
      y(
  ...
  _renderSkillsList(e) {
    ...
    let s = Z.join(r, t[n]),
      ...
      l = [];
    V.existsSync(s) &&
      V.readdirSync(s, { withFileTypes: !0 }).forEach((f) => {
        if (!f.isDirectory()) return;
        let _ = Z.join(s, f.name, "SKILL.md");
        if (!V.existsSync(_)) return;
        let g = V.readFileSync(_, "utf-8"),
          h = g.match(/^name:\s*(.+)$/m),
          b = g.split(`
`),
          E = b.findIndex((x) => /^description:/.test(x)),
          y = "";
        if (E >= 0) {
          let x = b[E].match(/^description:\s*(.+)$/);
          ...
          _.forEach((m) => {
            let w = m.name + (m.version ? " v" + m.version : ""),
              x = g
                ...
                : " [" + a("skills_user") + "]",
              S = m.desc || "",
              P = new R.Setting(E).setName(w + x).setDesc(S);
            ((P.settingEl.style.opacity = m.disabled ? "0.4" : "1"),
              P.addToggle((B) => {
                B.setValue(!m.disabled).onChange((I) => {
                  let M = !I,
                    D = m.content.match(/^disable-model-invocation:\s*(.+)$/m)
                      ...
                      : m.content.replace(
                          /^(---\r?\n)/,
                          `$1disable-model-invocation: ${M}
`
                        );
                  (V.writeFileSync(m.path, D, "utf-8"),
                    (m.disabled = M),
                    (m.content = D),
                    (P.settingEl.style.opacity = m.disabled ? "0.4" : "1"));
                });
        ...
        ((this._skillsCollapsed[v] || !1) &&
          ((E.style.display = "none"), (y.style.transform = "rotate(-90deg)")),
          b.addEventListener("click", () => {
            (E.style.display !== "none"
              ? ((E.style.display = "none"),
                (y.style.transform = "rotate(-90deg)"))
              : ((E.style.display = ""), (y.style.transform = "rotate(0deg)")),
  ...
  _renderOcrDetail(e) {
    ...
    if (t.activity_state === "running") {
      ...
      let k = this.plugin._ocrProgress,
        m = r.createDiv({ cls: "pf-ocr-progress-card" });
      if (k != null && k.total) {
        let x = a("ocr_progress")
            .replace("{current}", String(k.current))
            .replace("{total}", String(k.total)),
          S = k.key ? " \u2014 " + k.key : "";
        m.createEl("span", {
          cls: "pf-detail-progress",
          text: a("ocr_state_running") + " " + x + S,
        });
        let P = m.createDiv({ cls: "pf-activity-bar" }),
          B = Math.round((k.current / k.total) * 100);
        P.createDiv({
          cls: "pf-activity-bar-fill",
          attr: {
            style: `width: ${B}%`,
      ...
      w &&
        m
          ...
          .addEventListener("click", () => {
            var S, P;
            (S = w.stdin) != null && S.write
              ? (w.stdin.write(`PAPERFORGE_STOP
`),
                (this.plugin._ocrWasStopped = !0))
              : (P = w.kill) == null || P.call(w, "SIGINT");
          });
    } else if (o) {
      let k = n
  ...
  _renderAgentDetail(e) {
    ...
    if (
      ...
      for (let [E, y] of Object.entries(r)) {
        let v = h.createEl("option", { text: y, attr: { value: E } });
      ...
      (z(b, {
        ...
        onClick: () => {
          var y;
          let E = (y = this._agentPlatformDraft) != null ? y : i;
        ...
        z(b, {
          ...
          onClick: () => {
            ...
            let E = (v = this._agentPlatformDraft) != null ? v : i,
              y = V.existsSync(Z.join(this._getVaultBasePath(), n[E]));
            new R.Notice(
  ...
  _renderMemoryDetail(e) {
    ...
    let t =
        (b = (h = this._capabilityState) == null ? void 0 : h.memory) != null
          ? b
          : se("memory"),
      r = e.createDiv({ cls: "pf-module-body" }),
      n = (y = (E = t.reason) == null ? void 0 : E.code) != null ? y : "";
    if (
      (t.activity_state === "running" && t.user_state === "ready"
        ? (Y(r, "ready"),
          r.createEl("p", {
            text: (v = t.activity_label) != null ? v : a("cc_activity_running"),
            cls: "pf-status-ok",
          }))
        : n === "memory.disabled"
          ? (Y(r, "not_enabled"),
    ...
    for (let [m, w] of f) {
      let x = u.createDiv({ cls: "pf-sr-info-row" });
      (x.createEl("span", { cls: "pf-sr-info-label", text: m }),
        x.createEl("span", { cls: "pf-sr-info-value", text: w }));
    }
    if (n !== "ready" && n !== "memory.disabled") {
    ...
    let _ = r.createEl("details", { cls: "pf-sr-diagnostics" });
    (_.createEl("summary", {
      text: a("cc_diagnostic_toggle") || "Diagnostic Details",
    }),
      _.createDiv({ cls: "pf-sr-diagnostics-body" }).createEl("div", {
        text:
          "module: memory | state: " +
          t.capability_state +
          " | severity: " +
  ...
  _renderModuleDetailShell(e, t) {
    ...
    for (let x of s) {
      let S = l.createEl("option", { text: x.label, attr: { value: x.id } });
      S.selected = x.id === t;
    }
    ...
    let p =
        t === "agent"
          ...
          : (y = (E = this._capabilityState) == null ? void 0 : E[t]) != null
            ? y
            : se(t),
      u =
        (v = p.user_state) != null
    ...
    (_.createEl("span", {
      ...
      Y(_, u, this._getUserStateLabel(u)),
      f.createEl("p", {
    ...
    if (g && u !== "ready" && t !== "agent") {
      let x =
          "action_" +
          ((m = g.action_id) != null ? m : g.verb).replace(/[.-]/g, "_"),
        S = a(x),
        P =
          S !== x
            ? S
  ...
  _renderVectorReady(e, t) {
    ...
    (() => {
      ...
      let { current: _, total: g, key: h } = this.plugin._embedProgress,
        ...
        B = typeof f.message == "string" ? f.message : "",
        I = async (C) => {
          var J;
          if (C === "--resume" && k && !m) {
            let O = a("retrieval_rebuild_warning").replace("{n}", String(v));
            if (!confirm(O)) return;
          }
          if (C === "--force" && k && !m) {
            let O =
              ...
              ["embed", "build", C],
              {
                ...
                onStderr: (O) => {
                  (this.plugin._embedStderr || (this.plugin._embedStderr = ""),
                    (this.plugin._embedStderr += String(O)));
                },
                onError: (O) => {
                  ((this.plugin._embedProcess = null),
                    new R.Notice(
                      a("feat_build_failed") + ": " + (O.message || O)
                    ),
                    this.display());
                },
                onClose: (O) => {
            ...
            (this.plugin._embedPollInterval = setInterval(() => {
              this.plugin._embedPolling ||
                ...
                this._callPython(["embed", "status", "--json"], {
                  ...
                  onClose: (O, ae) => {
                    var oe;
                    if (((this.plugin._embedPolling = !1), O === 0 && ae))
                      try {
      ...
      switch (
        ...
        case "building": {
          ...
          if (
            ((Q.style.cssText = `width:${H}%; min-width:${_ > 0 ? "2px" : "0"};`),
            _ < g)
          ) {
            let O = C.createEl("div", {
              cls: "paperforge-progress-seg pending",
            });
            O.style.cssText = `width:${(100 - parseFloat(H)).toFixed(1)}%;`;
          }
          (o.createEl("span", {
        ...
        case "failed": {
          o.createEl("div", {
            ...
            text: a("retrieval_build_failed") + (B ? ": " + B : ""),
    ...
    (0, ne.execFile)(
      ...
      (n, i) => {
        ...
        (process.platform === "win32" &&
          f &&
          (u = Ge(Z.join(f, "Zotero", "Zotero", "Profiles"))),
          !u &&
            process.platform === "darwin" &&
            l &&
            (u = Ge(
        ...
        if (this._checkEl) {
          this._checkEl.setText(
            s.map((b) => `${_[String(b.ok)]} ${b.label}: ${b.detail}`).join(`
`)
          );
          let h = s.some((b) => !b.ok);
  ...
  _renderCard(e, t, r) {
    ...
    let h = g.createEl("div", { cls: "pf-cc-card-diagnostic-body" }),
      ...
      y = a("cc_activity_" + n.activity_state) || n.activity_state,
      v;
    try {
    ...
    (h.createEl("div", { text: `${a("cc_diag_module")}: ${n.module}` }),
      ...
      h.createEl("div", { text: `${a("cc_diag_activity")}: ${y}` }));
    let k = h.createEl("div");
    k.appendText(a("cc_diag_reason") + ": " + f + " ");
    let m = k.createEl("code", { text: n.reason.code });
    (h.createEl("div", {
  ...
  _renderControlCenter(e) {
    ...
    let i = (B = r.installation) != null ? B : se("installation"),
      s = (I = r.library) != null ? I : se("library"),
    ...
    (b.createEl("strong", { text: E }),
      b.createEl("span", { cls: "caption", text: y }));
    let v = _.createDiv({ cls: "pf-cc-summary-meta" }),
      k = v.createEl("span");
    (k.createEl("strong", { text: String(f) }),
      k.appendText(" " + (a("cc_needs_attention") || "item needs attention")));
      ...
      let C =
        D.id === "agent"
          ...
          : (M = r[D.id]) != null
            ? M
            : se(D.id);
      this._renderOverviewCard(S, D.id, D.label, C, F + 1);
    }
  }
...
var Le = class extends A.ItemView {
  constructor(e) {
    super(e);
  ...
  _fallbackFetchStats(e, t, r) {
    ...
    try {
      ...
      ((this._cachedStats = {
        ...
        exports: 0,
        bases: 0,
        ocr: { total: g, pending: b, processing: E, done: h, failed: y },
        path_errors: 0,
...
Make sure paperforge is installed and in your PATH.`,
  ...
  _renderGlobalMode() {
    ...
    let E = this._loadIndex(),
      y = E && E.items && E.items.length > 0;
    this._renderSystemStatusRow(
      f,
      "Index",
      y ? "healthy" : "missing",
      y ? E.items.length + " entries" : "formal-library.json not found"
    );
    let v =
      ...
      w = "No exports found";
    try {
      let T = nt.join(k, v, "PaperForge", "exports");
      if (xe.existsSync(T)) {
        let j = xe.readdirSync(T).filter((ue) => ue.endsWith(".json"));
        ((m = j.length > 0),
          (w = m ? j.length + " export(s)" : "No JSON exports"));
      }
    } catch (T) {}
    this._renderSystemStatusRow(
      f,
    ...
    ((P = tr(I)),
      (B =
        (M && ((Pt = M.summary) == null ? void 0 : Pt.reason)) ||
        (M && ((Ct = M.summary) == null ? void 0 : Ct.status)) ||
        "Unknown"),
      this._renderSystemStatusRow(
        f,
        "Memory Layer",
    ...
    if (F || !y || !m || !S) {
      ...
      let j = T.createEl("div", { cls: "paperforge-issue-list" });
      (F &&
        j.createEl("div", {
      ...
      let ue = T.createEl("div", { cls: "paperforge-issue-actions" }),
      ...
      Q.addEventListener("click", () => {
        ...
        let T =
            ((ue = _ == null ? void 0 : _.settings) == null
              ? void 0
              : ue.base_dir) || "Bases",
          j = this.app.vault.getAbstractFileByPath(T);
        if (j) {
          let le = null;
          if (
            (j.children &&
            ...
            ee && ee.openFile(le);
          } else new A.Notice("[!!] No .base file found in " + T, 6e3);
        } else new A.Notice("[!!] Base directory not found: " + T, 6e3);
      }));
    ...
    (J.createEl("span", {
      ...
      J.addEventListener("click", () => {
        let T = te.find((j) => j.id === "paperforge-sync");
        T && this._runAction(T, J);
      }));
    let O = H.createEl("button", { cls: "paperforge-contextual-btn" });
    (O.createEl("span", {
      cls: "paperforge-contextual-btn-icon",
      text: "\u229E",
    }),
      O.createEl("span", { text: "Run OCR" }),
      O.addEventListener("click", () => {
        let T = te.find((j) => j.id === "paperforge-ocr");
        T && this._runAction(T, O);
      }));
    ...
    (ae.createEl("span", {
      ...
      ae.addEventListener("click", () => {
        let T = te.find((j) => j.id === "paperforge-ocr-redo");
        T && this._runAction(T, ae);
      }));
  }
      ...
      (_.createEl("span", {
        ...
        _.addEventListener("click", () => {
          ...
          this.app.vault.getAbstractFileByPath(h)
            ...
            : new A.Notice("[!!] PDF not found: " + h, 6e3);
        }));
    }
    if (e.fulltext_path) {
      let _ = p.createEl("button", { cls: "paperforge-contextual-btn" });
...
`
    ...
    for (let _ of l) {
      ...
      ((h.checked = t[_.key] === !0),
        ...
        h.addEventListener("change", async () => {
          ...
          let E = h.checked;
          (await this.app.fileManager.processFrontMatter(b, (y) => {
            y[_.key] = E;
          }),
  ...
  _renderCollectionMode() {
    ...
    if (p + u + s + f > 0) {
      let m = r.createEl("div", { cls: "paperforge-ocr-section" }),
        w = m.createEl("div", { cls: "paperforge-collection-ocr-header" });
      ...
      let I = m.createEl("div", { cls: "paperforge-ocr-counts" }),
        M = [
      ...
      for (let F of M) {
        let D = I.createEl("div", { cls: "paperforge-ocr-count" });
        (D.createEl("div", {
          cls: "paperforge-ocr-count-value",
    ...
    let E = r.createEl("div", { cls: "paperforge-collection-actions" }),
      y = E.createEl("button", { cls: "paperforge-contextual-btn primary" });
    (y.createEl("span", {
      cls: "paperforge-contextual-btn-icon",
      text: "\u229E",
    }),
      y.createEl("span", { text: "Run OCR" }),
      y.addEventListener("click", () => {
        let m = te.find((w) => w.id === "paperforge-ocr");
        m && this._runAction(m, y);
      }));
    ...
    (v.createEl("span", {
      ...
      v.addEventListener("click", () => {
        let m = te.find((w) => w.id === "paperforge-sync");
    ...
    (k.createEl("span", {
      ...
      k.addEventListener("click", () => {
        let m = te.find((w) => w.id === "paperforge-ocr-redo");
  ...
  _renderVersionMode() {
    ...
    let l = i.createEl("div", { cls: "paperforge-version-paper-list" }),
      p = () => {
        l.empty();
        let y = this._versionFilter.toLowerCase(),
          v = this._versionPapers
            ? this._versionPapers.filter(
                (m) =>
                  !y ||
                  m.key.toLowerCase().includes(y) ||
                  m.title.toLowerCase().includes(y)
              )
            : [];
        if (v.length === 0) {
          l.createEl("div", {
      ...
      f = (y) => {
        if (
          ...
          u
            ...
            .createEl("span", { cls: "pf-title", text: y.title }),
          y.versions.length === 0)
        ) {
          u.createEl("div", {
            cls: "paperforge-meta",
        ...
        for (let m of y.versions) {
          let w = m.label === y.currentLabel,
            x = k.createEl("div", {
              cls:
                "paperforge-version-entry" +
                (w ? " paperforge-version-current" : ""),
            }),
            S = x.createEl("div", { cls: "paperforge-version-dot" }),
            P = x.createEl("div", { cls: "paperforge-version-content" }),
            B = P.createEl("div", { cls: "paperforge-version-label-row" });
          (B.createEl("span", {
          ...
          let I = m.created_at ? m.created_at.slice(0, 10) : "";
          P.createEl("div", {
            cls: "paperforge-meta",
            text: I + " \u2014 " + m.source,
          });
          let M = m.fulltext_size
            ? m.fulltext_size > 1024
              ? (m.fulltext_size / 1024).toFixed(0) + "KB"
              : m.fulltext_size + "B"
            : "";
          M && P.createEl("div", { cls: "paperforge-meta", text: M });
    ...
    (b.stdout.on("data", (y) => {
      ...
      b.on("close", (y) => {
        ...
        let v = E.join(""),
          ...
          m = v.lastIndexOf("}"),
          w = "";
        if (k !== -1 && m > k) w = v.slice(k, m + 1);
        else {
          let x = v.indexOf("["),
            S = v.lastIndexOf("]");
          x !== -1 && S > x && (w = v.slice(x, S + 1));
        }
        if (!w) {
        ...
        try {
          ...
          if (x && typeof x == "object" && "data" in x) {
            ...
            if (P && typeof P == "object") {
              let B = P;
              "matches" in B && Array.isArray(B.matches) && (S = B.matches);
            }
          }
          ...
          let S = x instanceof Error ? x.message : String(x);
          ((this._searchState = "internal-error"), this._renderSearchState());
        }
      }),
      b.on("error", (y) => {
  ...
  _renderSearchResultsList(e, t) {
    ...
    for (let i = 0; i < e.length; i++) {
      ...
      let f = typeof o.zotero_key == "string" ? o.zotero_key : "",
        ...
        h = _ || g;
      if (!h && f) {
        let y = this._getCachedIndex().find(
          (v) =>
            v !== null &&
            typeof v == "object" &&
            "zotero_key" in v &&
      ...
      (h
        ? p.addEventListener("click", (E) => {
            let y = E.ctrlKey || E.metaKey;
            this.app.workspace.openLinkText(h, "", y);
          })
      ...
      if (
        ...
        let E = o.score,
          y = typeof E == "number" ? E.toFixed(3) : String(E);
        b.createEl("span", {
        ...
        if (h !== 0) {
          ...
          (e.cmd === "repair" || e.cmd === "ocr") && h === 1
            ? (this._showMessage("[WARN] " + E, "running"),
              new A.Notice("[WARN] " + e.cmd + " partial: " + E, 8e3),
              this._fetchStats(!0))
            : (this._showMessage("[!!] " + E, "error"),
              new A.Notice("[!!] " + e.cmd + " failed: " + E, 8e3));
...
`);
            ...
            try {
              (JSON.parse(E),
                navigator.clipboard
                  .writeText(E)
                  .then(() => {
                    let y = `${b}s \u2014 ${E.length} chars copied`;
          ...
          let y =
              f.filter((k) => k.match(/updated \d+/)).pop() ||
              f[f.length - 1] ||
              "",
            v = `${b}s \u2014 ${y}`;
      ...
      u.on("error", (h) => {
        (t.removeClass("running"),
          ...
          new A.Notice("[!!] Cannot start: " + h.message, 8e3));
      }));
  }
  _showMessage(e, t) {
    this._messageEl &&
  ...
  async _loadPapers() {
    ...
    try {
      ...
      for (let y of E) {
        ...
        if (pe.existsSync(k))
          try {
            m = JSON.parse(pe.readFileSync(k, "utf-8"));
          } catch (S) {}
        let w = we.join(e, t.systemDir, "PaperForge", "ocr", v, "backups"),
          x = 0;
        (pe.existsSync(w) &&
          (x = pe
            .readdirSync(w)
          ...
          this.papers.push({
            key: v,
            title: (i = y.title) != null ? i : v,
            status:
              (o = (s = m.ocr_status) != null ? s : m.ocrStatus) != null
                ? o
                : "pending",
            pipelineVersion: (l = m.ocr_pipeline_version) != null ? l : "",
            lastRun:
            ...
            authors:
              (g =
                (_ = (f = y.authors) == null ? void 0 : f.join) == null
                  ? void 0
                  : _.call(f, ", ")) != null
                ? g
                : "",
            year: (h = y.year) != null ? h : "",
...
var at = class extends q.Plugin {
  ...
  async onload() {
    ...
    (this.addRibbonIcon(Ve, "PaperForge Dashboard", () => Le.open(this)),
    ...
    (0, Me.exec)(r, { timeout: 12e4, encoding: "utf-8" }, (n, i, s) => {
      ...
      try {
        ...
        (K.readdirSync(o).forEach((p) => {
          p.endsWith(".json") &&
            (l = Math.max(l, K.statSync(Ie.join(o, p)).mtimeMs));
        }),
