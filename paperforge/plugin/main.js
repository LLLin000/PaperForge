...
const memoryState = (() => {
  ...
  function getMemoryRuntime(vaultPath) {
    var paths = resolveVaultPaths(vaultPath);
  ...
  function isMemoryReady(vaultPath) {
    var s = getMemoryRuntime(vaultPath);
  ...
  function isHealthOk(vaultPath) {
    var s = getRuntimeHealth(vaultPath);
  ...
  return {
    resolveVaultPaths, readJSONFile,
    getMemoryRuntime, getVectorRuntime, getRuntimeHealth,
    isMemoryReady, isVectorReady, isHealthOk,
    getMemoryStatusText, getVectorStatusText,
...
class PaperForgeReadingCanvasView extends ItemView {
    ...
    _annotationTint(color) {
        if (!color || typeof color !== 'string') return 'rgba(255, 213, 79, 0.18)';
    ...
    _normalizeTextWithMap(text) {
        ...
        for (var i = 0; i < raw.length; i++) {
            ...
            if (pendingSpaceStart !== null) {
                normalized += ' ';
                map.push({ start: pendingSpaceStart, end: pendingSpaceStart + 1 });
    ...
    _highlightFirstTextMatchInRoot(root, selectedText, color) {
        ...
        var walker = doc.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
            acceptNode: function (node) {
                ...
                if (parent.closest && parent.closest('script, style, mark, .paperforge-reading-canvas-view')) {
