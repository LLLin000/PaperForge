> **Branch:** `master` | **Last Updated:** 2026-07-26
> **Active work:** Real test-vault UX repairs are deployed. Smart Retrieval credentials use endpoint/model profiles; its unified detail configuration now exposes API key, Base URL, and model behind one disclosure; live index rebuild reaches `memory.ready` for 593 papers. Setup Journey readiness refreshes Foundation and Library before Stage 4. Release/package/tag work remains deferred.
>
> ---
>
> **Current state:** A real Obsidian 1.12.7 audit against `D:\L\Med\test` found production-visible defects that prototype QA missed: the OCR workspace joined an already absolute PaperForge path and therefore loaded zero papers; the workspace opened in the right sidebar; Control Center strings used untranslated legacy keys; Smart Retrieval displayed a raw configuration key; and Obsidian’s `button:not(.clickable-icon)` rule overrode PaperForge’s primary-action styling.
>
> The deployed repair uses resolved index/OCR paths, opens OCR Workspace in a main tab, localizes the live Chinese control-center/detail strings, raises PaperForge action-button specificity, and keeps all setup choices in one journey: Foundation validates Python and installs only the PaperForge package; Library writes and verifies Zotero/vault paths; optional OCR, Smart Retrieval, and Agent settings expand and save in Stage 3. Retrieval API keys are profile-scoped by Base URL and embedding model, so changing either never reuses an unrelated credential.
>
> Next: owner accepts the real `D:\L\Med\test` surface; do not release until that acceptance and the normal issue workflow are complete.
## 1. Architecture
