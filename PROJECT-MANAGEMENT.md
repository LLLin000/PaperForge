...
| Knowledge & Retrieval Domain Cutover (2026-09-03) | **106 passed, 1 skipped** focused Python tests; **446 plugin tests passed; `npm run typecheck` clean**. Verified #137 frozen event vocabulary (`preflight`, `paper_settled`, `heartbeat`), real CLI embed build/resume progress streaming, single token cooperative cancellation (`rc=130`), dashboard search/retrieve (`--deep`, `--paper`, `--limit`), and Smart Retrieval memory build via `client.runAction()`. ✅ |
...
- **Worker-Backed Progress & Single Cancellation Token:** `ActionExecutionHooks` passes `stop_check`, `on_progress`, and `on_item_result` directly into `run_embedding_build`. `action run embed.build` and `embed.resume` stream real progress, paper settlement, and cooperative cancellation without stdout capture or competing token owners; `PAPERFORGE_STOP` returns standard `ACTION_CANCELLED` (`rc=130`).
- **Dashboard Search & Retrieval:** User-facing queries in `PaperForgeStatusView` route through `client.search(query, options)` and `client.retrieve(query, { deep: true })` with typed CLI options; direct `spawn` and manual JSON parsing are eliminated; missing client fails closed with `backend-unavailable`.
- **Smart Retrieval Control Panel:** Rebuild/resume actions in `PaperForgeSettingTab` route through `client.runAction()`, streaming progress into the capability envelope and wiring the Stop button to `client.cancelActiveOperation()`.
- **Verification:** Focused Python suite **106 passed / 1 skipped**; Plugin suite **446/446 passed**; `npm run typecheck` clean.
...
| 2026-09-03 | Ticket 05 Knowledge & Retrieval thin-client cutover | Frozen #137 NDJSON vocabulary across Python and TS; wired ActionExecutionHooks into embedding service with worker-backed progress and single cancellation token; routed dashboard search/retrieve and Smart Retrieval build/resume to PaperForgeClient. Focused evidence: **106 passed, 1 skipped** Python; **446 plugin**; typecheck clean. | §2.25, `.scratch/thin-client/issues/05-retrieval-and-knowledge-cutover.md` |
