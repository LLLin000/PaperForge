> **Branch:** `master` | **Last Updated:** 2026-08-21
>
> **Active work:** `EXECUTABLE_FROZEN a42f8bb7` / `PROTOCOL_DOCS 781910f3` remain the RC baseline; the new read-only render consistency investigation is uncommitted and scoped to existing OCR truth, inventory, assets, render artifacts, and the unified `probe lineage` projection. **#81 OPEN / owner gate; render reconcile NOT STARTED.**
>
> ---
>
> **Current state:** Recovery and RC certification records remain valid. The 965-paper render audit completed read-only: 189 CLEAN, 776 DEGRADED, 0 FAILED, 5527 issues; 5304 classify to `render_layer`. Controlled renderer experiments now isolate the investigation to existing materialization inputs/outputs; next action is to explain the historical asset-write/crop gap without adding a second detector or mutating production artifacts.
...
0.1. ⏳ **Render materialization integrity investigation** — explain the existing `images/blocks → assets/{figures,tables} → render/*.md` gap using the current audit report and existing renderer/resolver/probe lineage outputs; no duplicate detector, no `render reconcile`, no production mutation.
...
|------|---------|-------------|------------------|
| 2026-08-21 | **Read-only render consistency + materialization provenance** | Audited 965 papers: 189 CLEAN, 776 DEGRADED, 0 FAILED, 5527 issues; 5304 classified as render-layer. Existing object seam now preserves the crop bool contract while emitting additive stage/reason provenance; audit projects it and `probe lineage` projects the persisted report. A 10/10/10 temporary A/B/C matrix found raw-locator failures, resolved-PDF successes, one page-boundary failure, and two source-asset-unavailable table cases. No production artifacts changed; reconcile remains deferred. | §2.13, §4 |
