...
| 53 | — | Embed noop settlement lied (`progress current: 0,0`, `papers_skipped: 0`, `paper_settled outcome=succeeded` for skipped papers); unknown memory action IDs were silently substituted with `memory.build`; Stop button had a legacy `embedController.busy/stop` fallback bypassing client ownership | Event wiring + fail-closed dispatch | `paper_settled(key,status)` moved from `_on_progress` to `_on_item_result` in the embed hooks; noop branches increment `processed_count`/`papers_skipped`; un-mocked CLI test proves hash-skip noop stream truth; `_runAllowedDispatch("memory")` else-branch now Notice + re-probe (no substitution, regression typed `satisfies ActionPrimary`); `_renderMemoryDetail` Stop bound only to `client.isOperationActive()`; legacy tests pinning execFile memory build and embed-controller Stop deleted. Python focused 169 passed; plugin 446/446; typecheck clean. | — |
...
| 2026-09-05 | Ticket 05 corrective round 2 — noop settlement, fail-closed entrypoint, Stop ownership | `paper_settled(key,status)` moved from `_on_progress` to `_on_item_result`; noop branches restore counters; un-mocked real-service CLI test asserts clean NDJSON with `progress.current==1`, single `paper_settled outcome=noop`, single `item_result status=noop`, `papers_skipped==1`; `_runAllowedDispatch("memory")` unknown IDs fail closed (Notice + re-probe, no `memory.build` substitution); `_renderMemoryDetail` Stop bound solely to `client.isOperationActive()`, legacy embed-controller fallback and stale tests deleted. Evidence: Python focused **169 passed**; plugin **446/446**; typecheck + ruff (changed files) clean. | §2.3 Fix 53, `.scratch/thin-client/issues/05-retrieval-and-knowledge-cutover.md` |
