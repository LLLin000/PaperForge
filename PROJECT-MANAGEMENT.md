> **Branch:** `master` | **Last Updated:** 2026-07-31
> **Active work:** Retrieval optimization series complete. 14 commits: 6 missing indexes, KNN probe fast path, fulltext copy guards, force rebuild back to conservative backup protocol, **#116 per-paper incremental memory build** (closed after 3 GPT review cycles, 12 P0 fixes). 39 tests pass including 7 OCR integration tests. **Next: #117 atomic shadow vector rebuild.**
>
> ---
>
> **Current state:** Retrieval v2 optimization series landed on `master`. `build_from_index()` now does per-paper metadata diff (materialized `paper_state_hash`, 2-SQL constant-query diff), per-paper OCR unit rebuild with vector invalidation, orphan-safe reading/correction imports, and fresh/destructive/additive migration routing. `get_embed_status()` KNN probe is opt-in (6.6s → 0.46s). Force rebuild reverted from pseudo-shadow swap to conservative `sqlite3.backup()` + in-place protocol (5e607fd2).
>
> Next: implement #117 Atomic Shadow Vector Rebuild (workers write `.db.build`, checkpoint → verify → single `os.replace()` publish).
## 1. Architecture
