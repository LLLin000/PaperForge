...
| 2026-08-11 | T5 — reconcile module + registry closure (#166) | `paperforge/reconcile.py`: three-layer observation (global desired → global substrate → per-paper lineage), global repair frontier first (exactly one global intent on substrate incompatibility; per-paper facets blocked_global), minimal per-paper frontier (prereqs-satisfied first-layer deficits only), scope merging by canonical action, single intents projection onto PFResult.next_actions; pure derivation + emission (idempotent, no retry tables — W2 gate lives at trigger #158); unknown facets fail closed; registry closed: ocr.run + embed.build registered (credential preflights); CLI `reconcile [--scope --key --json]`; 12 new tests, full suite 3057 passed; commit `d9408411`; #166 closed | §9.N |
