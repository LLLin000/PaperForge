...
>
> **Active work:** Reconcile-only R promotion has the cross-object ownership gate, destination CAS, postcondition commit gate, durable journal recovery, and explicit CLI scope. A 12-object disposable stratified set produced 11 successful promotions and 1 expected page-range failure; all 11 second promotes were no-ops. No production batch promotion is authorized.
>
> ---
>
...
| Reconcile-only R promoter (2026-08-26) | **114 passed, 1 warning** across R promoter, audit, reconciliation, lineage, CLI dispatch, and command docs; targeted Ruff `E,F,I,UP` clean. Cross-canonical claim conflict, destination CAS, postcondition rollback, durable journal recovery, four hard-kill injection points, explicit scope, 12 disposable objects tested (**11 promoted, 1 expected page-range failure**), and 11 second-promote no-ops passed. No production write executed. ✅ |
...
The promoter validates: manifest freshness, canonical occurrence exactly one, duplicate-identity absence, cross-canonical `(page, block_id, bbox)` ownership, exact ordered refs, destination CAS preconditions, fixed staged/production paths, decodable staged image, staged dimensions/hash, markdown link resolution, staging provenance, and plan hash. It uses a paper-scoped cross-process lock, durable transaction journal, atomic per-file replacement, post-audit commit verification, rollback on failure, and idempotent promotion.

Exception-injection rollback and four hard-kill recovery points are verified. A 12-object disposable stratified set produced 11 successful promotions and 1 expected page-range failure; all 11 second promotes were no-ops. This is not a production rollout approval; no production write executed. Evidence: `project/current/render-reconciliation/r-canary-results.json`.
### M4 RC Gate Policy
...
0.1. ⏳ **Reconcile-only render-result hardening** — identity census and blocking are complete; D1 P0 gates, durable recovery, and explicit scope are verified. A 12-object disposable set produced 11 successful promotions and 1 expected page-range failure; all 11 second promotes were no-ops. No front OCR/render code changed and no production write ran. **Next: explicit owner authorization only; no batch rollout.**
