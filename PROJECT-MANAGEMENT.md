> **Branch:** `master` | **Last Updated:** 2026-09-01
>
> **Active work:** Reconcile-only R promotion is **disposable-canary PASS but production-canary BLOCKED**. R P0-A/P0-B fail-closed audit, recovery-CAS, exact journal-path, committed-cleanup, and report-refresh handling are implemented and regression-covered. Fulltext malformed-authority mutation is blocked; a read-only 969-paper census found zero `R_CONTENT_UNVERIFIED` findings. Semantic coverage remains report-only and production rollout still requires owner authorization.
>
> ---
>
> **Current state:** The front OCR/render pipeline remains unchanged. The independent reconcile layer detects and blocks duplicate canonical identities, cross-canonical claims, ambiguous provenance, malformed recovery journals, unsafe promotion IDs, malformed fulltext authority, and raw symlink paths; it stages selected R plans in isolation and protects explicit promotion with manifest/CAS/journal gates. A committed promotion preserves production through cleanup/report-refresh failures; an original-call cleanup `OSError` now returns `committed=true, cleanup=pending` with the journal retained for recovery, and final audit status is distinct from report-write status. No production materialization or canonical inventory mutation was performed.
