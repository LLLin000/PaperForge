> **Branch:** `master` | **Last Updated:** 2026-09-03
>
...
> ---
>
> **Current state:** The front OCR/render pipeline remains unchanged. The independent reconcile layer detects and blocks duplicate canonical identities, cross-canonical claims, ambiguous provenance, malformed recovery journals, unsafe promotion IDs, malformed fulltext authority, and raw symlink paths; it stages selected R/P plans in isolation and protects explicit R promotion and human-authorized P acceptance with lock/CAS/journal gates. P acceptance is the only path that may add a reviewed canonical object, and it requires the exact SHA-256 token for the reviewed `final-plan.json` before atomically updating inventory, render, provenance, and proposal state with post-audit rollback before commit. A committed action preserves production through cleanup/report-refresh failures. No production materialization or canonical inventory mutation is authorized.
...
| Reconcile-only R promoter (2026-08-26; pre-closure snapshot) | **114 passed, 1 warning** across R promoter, audit, reconciliation, lineage, CLI dispatch, and command docs; targeted Ruff `E,F,I,UP` clean. Cross-canonical claim conflict, destination CAS, postcondition rollback, durable journal recovery, four hard-kill injection points, explicit scope, 12 disposable objects tested (**11 promoted, 1 expected page-range failure**), and 11 second-promote no-ops passed. At this snapshot, production canary remained blocked on audit-`FAILED` handling and recovery CAS; the 2026-08-31 closure is recorded below. ✅ (disposable evidence only) |
| Reconcile safety closure (2026-08-31) | **108 passed, 1 warning** across R promotion, render audit, figure reconciliation, fulltext authority, and lineage projection; focused Ruff `E,F,I,UP` clean. Added fail-closed malformed provenance/journal gates, deterministic Windows JSON hashing, rollback CAS, unsafe-ID rejection, report-level `R_CONTENT_UNVERIFIED`, bounded-candidate page validation, and fulltext source/identity blocking. ✅ (disposable fixtures only) |
| Reconcile recovery/fulltext follow-up (2026-09-01; pre-second-review snapshot) | **114 passed, 1 warning** across R promotion, render audit, figure reconciliation, fulltext authority, and lineage projection; focused Ruff `E,F,I,UP` clean. Added exact journal transaction/destination/backup allowlists, schema-v2 enforcement, malformed canonical inventory blocking, orphan negative coverage, and explicit post-commit report-refresh failure status. ✅ (disposable fixtures only) |
| Reconcile cleanup hardening and census (2026-09-01) | **117 passed, 2 skipped, 1 warning** across R promotion, render audit, figure reconciliation, fulltext authority, and lineage projection; targeted Ruff `E,F,I,UP`, `py_compile`, and `git diff --check` clean. Added committed-journal cleanup recovery without backup requirements, structured `cleanup=pending` on post-commit cleanup `OSError`, raw destination/backup symlink rejection, failed final audit-state separation, and a read-only 969-paper census with **0 affected papers / 0 affected objects**. ✅ (disposable fixtures only) |
| Reconcile P authority acceptance (2026-09-02) | **126 passed, 2 skipped, 1 warning** across proposal acceptance, R promotion, render audit, figure reconciliation, fulltext authority, and lineage; targeted Ruff `E,F,I,UP`, `py_compile`, and `git diff --check` clean. Added shared writer locking, exact human-reviewed `final-plan.json` SHA-256 binding, strict reviewed-plan snapshots, true final-plan provenance, five-target journaled atomic writes, CAS rollback, committed cleanup recovery, and final audit/report refresh. ✅ (disposable fixtures only) |
| Stale render-consistency report invalidation (2026-09-02) | **132 passed, 2 skipped, 1 warning** across OCR render output, render audit, and lineage projection; `render_audit.py` targeted Ruff, both changed Python modules `py_compile`, and `git diff --check` clean. Automatic audit exceptions/non-object results now replace old persisted status with a structured `FAILED` report, with best-effort removal if replacement fails. ✅ (disposable fixtures only) |
...
| OCR Workspace & Processing Domain Cutover (2026-09-03) | **103 passed, 1 skipped** focused Python tests; **63 plugin tests passed; `npm run typecheck` clean**. Corrective coverage proves real CLI rebuild progress/item-result ordering, worker-safe-point `PAPERFORGE_STOP` cancellation (`rc=130`), wire `event.phase` consumption, fail-closed client absence, separate multi-key OCR argv, and descriptor-cache refresh. No OCR/render materialization ran. ✅ |
| Knowledge & Retrieval Domain Cutover (2026-09-03) | **107 passed, 1 skipped** focused Python tests; **448 plugin tests passed; `npm run typecheck` clean**. Verified #137 frozen event vocabulary (`preflight`, `paper_settled`, `heartbeat`), real un-mocked CLI embed build/resume progress streaming, silent service mode without non-JSON stdout pollution or legacy `EMBED_PROGRESS` tokens, pre-publish cancellation safe point, single token cooperative cancellation (`rc=130`), dashboard search/retrieve (`--deep`, `--paper`, `--expand`, `--no-expand`), domain-safe action allowlist fail-closed, and Smart Retrieval memory build via `client.runAction()`. ✅ |
...
| **OCR Workspace / Processing UI** | ✅ `PaperForgeClient` lineage + typed OCR rows, descriptor-gated `run`/`rebuild_derived`, client-owned NDJSON progress and cooperative cancellation |
...
| 53 | — | P proposal acceptance could partially write canonical and render outputs without a shared production transaction | Authority transaction | `accept-proposal` now requires the exact human-reviewed `final-plan.json` SHA-256 token, a live reviewed snapshot, and exact staged paths; validates plan-owned member refs, writes inventory/JPG/Markdown/provenance/proposal state through one lock/CAS/journal transaction, rolls back before commit on failed post-audit, and preserves committed output through cleanup/report-refresh failure. | — |
...
| 50 | — | A new render was written while the automatic consistency audit raised; the previous CLEAN/DEGRADED report remained readable by lineage | The shared render-output boundary treated audit exceptions as warning-only without invalidating persisted report authority | Write an atomic `FAILED` execution-failure report on audit exception/non-object; if replacement fails, remove the old report best-effort; add regression coverage for the writer and lineage projection | — |
| 51 | — | OCR Workspace read/action path bypassed the thin-client boundary; review then found fabricated MockTransport progress, dropped cancellation/hooks, `event.status` phase drift, fallback client construction, collapsed OCR keys, and stale action descriptors | Thin-client cutover | Table rows use `probe("lineage")` + `queryOcrPapers()`; Run OCR, Redo, and Rebuild Derived use descriptor-gated `runAction`; ActionExecutionHooks carry cancellation/progress/item results into real workers; Workspace consumes `event.phase`, fails closed without the shared client, passes separate OCR key args, and clears descriptors on refresh. | — |
| 52 | — | Smart Retrieval and Dashboard search bypassed thin client; embed streaming buffered stdout and had duplicate cancellation token owners; TS NDJSON parser rejected standard vocabulary | Thin-client cutover | Frozen #137 vocabulary in TS and Python; wired ActionExecutionHooks into `run_embedding_build` with worker progress/cancellation; routed dashboard search/retrieve and SettingTab build/resume to `client.runAction()`; added real CLI and plugin regressions. | — |
| 53 | — | Embed noop settlement lied (`progress current: 0,0`, `papers_skipped: 0`, `paper_settled outcome=succeeded` for skipped papers); unknown memory action IDs were silently substituted with `memory.build`; Stop button had a legacy `embedController.busy/stop` fallback bypassing client ownership | Event wiring + fail-closed dispatch | `paper_settled(key,status)` moved from `_on_progress` to `_on_item_result` in the embed hooks; noop branches increment `processed_count`/`papers_skipped`; un-mocked CLI test proves hash-skip noop stream truth; `_runAllowedDispatch("memory")` is an exhaustive exact-ID chain (`memory.build`/`memory.rebuild` dispatch, unknown IDs Notice + re-probe, no substitution) whose dispatched branch `return`s so legal actions never fall through to the unknown-pair probe; production-entry `it.each` regression proves legal IDs run and unknown IDs don't; `_renderMemoryDetail` Stop bound only to `client.isOperationActive()`; legacy tests pinning execFile memory build and embed-controller Stop deleted. Python focused 169 passed; plugin 448/448; typecheck clean. | — |
| 54 | — | Library sync and Render Quality surfaces bypassed the thin client: manual sync ran a raw `sync --json` child process, Library facts were static placeholders, and render audit / R promotion / P acceptance had no UI surface at all | Thin-client cutover | `_runManualSync` routes through `client.sync()` with exit-code forwarding (#78); Library workbench renders probe-owned `details.paper_count` (new structured field on ready/stale envelopes) and the maintenance envelope's orphan count; Render Quality drawer inside paper technical details uses `renderAudit(key)`, new `renderReconcileStaging(key)`, per-object `promoteR`, and per-card `acceptProposal` forwarding the exact reviewed SHA-256; committed mutations invalidate the staging snapshot. Plugin 455/455; Python focused 193 passed; typecheck clean. | — |
...
The census artifact is `project/current/render-reconciliation/duplicate-id-census.json`. Verification: **82 focused tests passed, 1 warning**; Ruff `E,F,I` clean. D1 promotion is implemented separately; human proposal acceptance remains a separate authority action.

### 2.17 R promoter (2026-08-26 — IMPLEMENTED; DISPOSABLE CANARY PASS, PRODUCTION CANARY BLOCKED)
...
The promoter validates: manifest freshness, canonical occurrence exactly one, duplicate-identity absence, cross-canonical `(page, block_id, bbox)` ownership, exact ordered refs, destination CAS preconditions, safe object IDs and fixed staged/production paths, decodable staged image, staged dimensions/hash, markdown link resolution, unambiguous staging/production provenance, and plan hash. It uses a paper-scoped cross-process lock, durable transaction journal with validated shape and CAS recovery, deterministic atomic JSON output, atomic per-file replacement, post-audit commit verification, rollback-on-failure, and idempotent promotion. These checks are relationship-exact; they do not prove semantic Figure coverage or validate a pre-existing legacy image with no authoritative output hashes.

Exception-injection rollback and four hard-kill recovery points are verified. A 12-object disposable stratified set produced 11 successful promotions and 1 expected page-range failure; all 11 second promotes were no-ops. A fresh isolated `2ZBAK9VY/figure_003` run failed only its own missing/invalid staged-source gates, without stale-destination contamination. This is not a production rollout approval; no production write executed. Evidence: `project/current/render-reconciliation/r-canary-results.json`.


### 2.18 D1 and P-lane status

- **R P0-A — closed:** `_promotion_audit()` converts audit exceptions/non-object results to `FAILED`; promotion rejects failed pre-audit before any write and failed post-audit before the committed journal marker, then records rollback-audit state.
- **R P0-B — closed:** pending journals carry per-destination `before` and transaction-output existence/hash; recovery refuses overwrite when current state matches neither and retains the journal for operator recovery.
- **R detection boundary — partial:** report and dry-run now emit/block provenance-missing, provenance-mismatch, malformed, and duplicate-path cases as `R_CONTENT_UNVERIFIED`/provenance blockers. Semantic panel coverage remains unproven and report-only; do not infer correctness from bbox/pixel ratios or merge `table_html`.
- **P corrective — closed:** `_bounded_slot_dry_run()` derives anchors from unique, non-conflicted canonical rows in the `main` namespace; candidates with missing page coordinates are blocked instead of crashing.
- **Fulltext authority — closed:** canonical figure IDs are deduplicated, duplicate canonical identities produce an F0 issue, and all fulltext patches are suppressed until the inventory is unambiguous; unavailable fulltext is also blocked.
- **Additional safety audit — closed:** malformed journal shape/state, unsafe promotion IDs, duplicate provenance paths, and Windows newline-dependent JSON hashes are covered by fail-closed gates and focused regressions.
- **Remaining production gate — open:** no production batch rollout; owner authorization and semantic coverage adjudication remain required.

### 2.19 Reconcile follow-up safety closure (2026-09-01 — COMPLETE)

- **Journal recovery P0 — closed:** recovery requires schema version 2, an exact `.paperforge-r-promotion/<32-hex-id>` transaction path, a non-symlink transaction root/entry, allowlisted destinations, unique destinations, and integer `.bak` files inside the exact transaction directory.
- **Fulltext authority P0 — closed:** missing, unreadable, non-object, non-list, or malformed canonical figure inventory produces `F0_CANONICAL_AUTHORITY_UNAVAILABLE`, sets `mutation_blocked`, and emits no patches, including orphan/reserved embed candidates.
- **Post-commit reporting P1 — closed:** final `render.consistency.json` refresh is isolated from the committed transaction; a refresh exception returns `ok=true`, `committed=true`, `report_refresh=failed`, and never rolls back committed artifacts.
- **Verification:** focused reconcile suite **114 passed, 1 warning**; targeted Ruff `E,F,I,UP` and `py_compile` clean; no production write or front OCR/render change.

### 2.20 Committed recovery cleanup closure and unverified-content census (2026-09-01 — COMPLETE)

- **Committed cleanup P0 — closed:** `state=committed` recovery validates journal/path namespace safety but does not require backup files to remain; cleanup is best-effort, the journal is removed, and committed production artifacts are preserved. This closes the crash window between backup deletion, transaction-directory removal, and journal deletion.
- **Current-call cleanup P1 — closed:** after the commit marker, `_finish_transaction()` `OSError` is converted to `cleanup=pending` plus `cleanup_error`; the result remains `ok=true`, `committed=true`, final report refresh still runs, and the committed journal remains available for the next recovery.
- **Raw symlink P1 — closed:** destination and backup checks inspect raw path components before resolution; symlinked production paths, transaction paths, and backups fail closed.
- **Final audit-state P1 — closed:** a report write that returns audit `state=FAILED` is represented as `report_refresh=written` plus `report_audit_state=FAILED`; exceptions/non-object results remain `report_refresh=failed`.
- **Read-only census:** `D:\L\OB\Literature-hub` scanned **969** OCR paper directories with `write_report=False`: **0 affected papers, 0 affected objects, 0 audit failures**, and no reason/canonical-presence findings.
- **Verification:** focused reconcile suite **116 passed, 2 skipped, 1 warning**. The two symlink tests were skipped because Windows returned `WinError 1314` (missing symlink privilege); targeted Ruff, `py_compile`, and `git diff --check` passed. No production write or front OCR/render change.

### 2.21 P authority acceptance hardening (2026-09-02 — COMPLETE; PRODUCTION CANARY BLOCKED)

`paperforge render reconcile --json` emits `p_details[].final_plan_hash`. `paperforge render accept-proposal KEY LABEL --plan-hash HASH` now uses the same paper-scoped writer lock as R promotion and accepts only the exact reviewed plan bytes. It fails closed on missing/invalid/unmatched review tokens, missing/stale reviewed snapshots, unsafe paths, malformed plans, invalid staged images/Markdown, duplicate identity or asset claims, and malformed canonical authority. The exact `member_refs` from the reviewed plan are preserved; no rematching is performed.

The action prepares one schema-v2 journal for the canonical JPG, Markdown, `figure_inventory.json`, materialization provenance, and proposal report; writes each target atomically; verifies the post-write audit before the committed marker; and restores all snapshots on failure. The committed marker makes production authoritative, so cleanup/report-refresh failures are surfaced without rollback. `accepted_from.final_plan_hash` and provenance store the direct SHA-256 of `final-plan.json`, and the final consistency report is refreshed after commit.

Verification: **121 passed, 2 skipped, 1 warning** across the focused reconcile suite; targeted Ruff `E,F,I,UP`, `py_compile`, and `git diff --check` passed. The two symlink tests remain skipped on Windows `WinError 1314`; no production write or front OCR/render change.

### 2.22 Exact human-reviewed plan binding (2026-09-02 — COMPLETE; PRODUCTION CANARY BLOCKED)

`render reconcile --json` now emits each staged P plan's `final_plan_hash`,
computed from the exact bytes of `final-plan.json`. `accept-proposal` requires
that token and scans all safe KEY/LABEL staging candidates by SHA-256 instead
of selecting the newest mtime; an unmatched token returns
`STALE_REVIEWED_PLAN`. Duplicate candidates with identical plan bytes use a
stable path tie-break and do not weaken the live snapshot, CAS, journal, or
post-audit gates.

Verification: **126 passed, 2 skipped, 1 warning** across the focused reconcile
suite; targeted changed-module Ruff `E,F,I,UP`, `py_compile`, and
`git diff --check` passed. The two symlink tests remain skipped on Windows
`WinError 1314`; no production write or front OCR/render change.


### 2.23 Stale render-consistency report invalidation (2026-09-02 — COMPLETE; PRODUCTION CANARY BLOCKED)

The automatic audit remains diagnostic-only, but its persisted report is the authority projected by lineage. A new render that raises or returns a non-object from the audit now replaces any prior `render.consistency.json` with a structured `state=FAILED` execution-failure report. If that replacement itself fails, the writer attempts to remove the prior report rather than leave old `CLEAN` or `DEGRADED` state available as current.

Verification: **132 passed, 2 skipped, 1 warning** across the focused reconcile suite; `render_audit.py` targeted Ruff, both changed Python modules `py_compile`, and `git diff --check` passed. The two symlink tests remain skipped on Windows `WinError 1314`; no production write or front OCR/render corpus change.

### 2.24 OCR Workspace and Processing Domain Cutover (2026-09-03 — COMPLETE; corrective closure)

The OCR Workspace derives paper rows from `PaperForgeClient.probe("lineage")` and the typed `queryOcrPapers()` result. Backend lineage controls stale/update, incomplete, failed, queued, and processing projections; the OCR probe supplies global activity truth. Run OCR and user-facing Redo map to canonical `ocr.run`, while Rebuild Derived maps to `ocr.rebuild_derived`; descriptors gate availability and confirmation before the client builds the scoped action request. NDJSON phase, progress, item-result, terminal, and cancellation events drive the activity panel. Stop is enabled only while this client owns the active operation handle.

The Workspace no longer resolves Python runtimes or imports `execFile`/`OcrProcessController`; it fails closed when the shared client is unavailable and clears dynamic action descriptors on read-model refresh. `ocr.redo` remains an internal legacy command outside the production Workspace path. Python action streams pass cooperative cancellation and worker callbacks through `ActionExecutionHooks`, so rebuild and OCR progress/item-result events come from real workers rather than transport fabrication.

Verification: **103 passed, 1 skipped** focused Python tests; **63 plugin tests passed** across client/OCR regressions; `npm run typecheck` clean. Real CLI tests cover rebuild start → phase/progress/item-result → exactly one terminal and `PAPERFORGE_STOP` → worker-safe-point cancellation with `rc=130`. No OCR/render materialization was run.

### 2.25 Knowledge & Retrieval Domain Cutover (2026-09-03 — COMPLETE)

The Knowledge & Retrieval domain is now cut over to `PaperForgeClient`:
- **NDJSON Vocabulary Harmonization:** Python and TypeScript agree on the frozen #137 vocabulary: `start`, `preflight`, `phase`, `progress`, `paper_settled`, `heartbeat`, `item_result`, and terminal `result`, `error`, `cancelled`. TS `NdjsonStreamParser` accepts all 10 events without protocol failure.
- **Worker-Backed Progress & Single Cancellation Token:** `ActionExecutionHooks` passes `stop_check`, `on_progress`, and `on_item_result` directly into `run_embedding_build`. `action run embed.build` and `embed.resume` run in silent service mode (`json=True, emit_events=False, result_sink`), eliminating stdout capture, legacy `EMBED_PROGRESS:` tokens, and competing token owners; `PAPERFORGE_STOP` returns standard `ACTION_CANCELLED` (`rc=130`).
- **Pre-publish Cancellation Safe Point:** `run_embedding_build` checks `_is_stopped()` after candidate verification and before irreversible `_shadow.publish()`, preventing cancelled runs from publishing.
- **CLI Wire Parity:** Added `--no-expand` to `retrieve` CLI to match `RetrieveOptions.expand: false`.
- **Dashboard Search & Retrieval:** User-facing queries in `PaperForgeStatusView` route through `client.search(query, options)` and `client.retrieve(query, options)` with typed CLI options; `unwrapMatches()` encapsulates envelope handling; missing client fails closed with `backend-unavailable`.
- **Smart Retrieval Control Panel:** Rebuild/resume actions in `PaperForgeSettingTab` route through `client.runAction()`, streaming progress into the capability envelope, strictly failing closed on cross-domain action IDs, and wiring the Stop button only when `client.isOperationActive()`.
- **Verification:** Focused Python suite **107 passed / 1 skipped**; Plugin suite **448/448 passed**; `npm run typecheck` clean.
### M4 RC Gate Policy
...
0.2. ⏳ **P authority acceptance hardening** — `accept-proposal` now shares the paper writer lock and schema-v2 journal/CAS/recovery contract with R, requires a live reviewed snapshot and direct final-plan hash, preserves plan-owned member refs, atomically updates the five P destinations, rolls back on failed post-audit, and refreshes the final report. Disposable safety evidence is **4/4 passed**; production remains owner-blocked.
0.3. ✅ **Ticket 04 — OCR Workspace & Processing Domain Cutover** — corrective closure complete: real CLI streams now carry worker-backed progress/item results and cooperative cancellation; the Workspace consumes `event.phase`, fails closed without the shared client, passes multi-key OCR argv correctly, and refreshes dynamic descriptors. Focused Python/plugin gates and typecheck pass. `ocr.redo` stays internal-only; user-facing Redo uses canonical `ocr.run`.
0.4. ✅ **Ticket 05 — Knowledge & Retrieval Domain Cutover** — thin-client search/retrieve and memory/embed build cutover complete; worker-backed progress and single cancellation token verified; #137 NDJSON vocabulary frozen across Python and TS.
0.5. ✅ **Ticket 06 — Library & Render Quality Domain Cutover** — Library sync and Render Quality drawer route through `PaperForgeClient` (`sync`/`renderAudit`/`renderReconcileStaging`/`promoteR`/`acceptProposal`); R/P CAS/journal/plan-hash semantics stay frozen backend contracts; orphan/paper counts render from probe envelopes without client-side database scans.
...
| 2026-09-02 | Bind P acceptance to the reviewed plan bytes | A current snapshot and stable proposal ID do not prove which of multiple same-label staging runs the user inspected. `render reconcile` now emits SHA-256 of each exact `final-plan.json`; `accept-proposal` requires that token and selects only matching plan bytes, so human approval cannot silently drift to a newer plan. |
| 2026-09-02 | Invalidate persisted render audit on automatic failure | A new render changes the underlying artifacts; leaving the prior `CLEAN`/`DEGRADED` report would let lineage project stale state. The shared output boundary therefore replaces it with `FAILED` before returning its diagnostic-only outcome. |
| 2026-09-02 | Reconcile Core Frozen & Backend Contract Mapping | Freeze Reconcile mutation engine; publish `docs/RECONCILE-EXTENSION-CONTRACT.md` (8 extension rules, minimal finding schema, default-blocked, observation/mutation separation). Establish Python Authority as SSOT across 5 backend contracts and 6 product projection domains in `docs/BACKEND-FRONTEND-CONTRACT-MAPPING.md`. Shift active work to backend surface rationalization and frontend thin-client mapping. |
| 2026-09-03 | Ticket 04 OCR Workspace thin-client cutover and corrective closure | Initial cutover moved OCR reads/actions/progress/cancellation to `PaperForgeClient`; review found transport-fabricated worker events, dropped stop hooks, phase-field drift, fallback client construction, collapsed multi-key argv, and stale descriptors. Corrective patch wires `ActionExecutionHooks` into real workers, consumes `event.phase`, fails closed, refreshes descriptors, and adds real CLI regressions. Focused evidence: **103 passed, 1 skipped** Python; **63 plugin**; typecheck clean. | §2.24, `.scratch/thin-client/issues/04-ocr-workspace-cutover.md` |
| 2026-09-03 | Ticket 05 Knowledge & Retrieval thin-client cutover | Frozen #137 NDJSON vocabulary across Python and TS; wired ActionExecutionHooks into embedding service with worker-backed progress, single cancellation token, and pre-publish safe point; eliminated legacy EMBED_PROGRESS tokens and silenced stdout in service mode; added retrieve --no-expand parity; routed dashboard search/retrieve and Smart Retrieval build/resume to PaperForgeClient with strict domain allowlist. Focused evidence: **107 passed, 1 skipped** Python; **448 plugin**; typecheck clean. | §2.25, `.scratch/thin-client/issues/05-retrieval-and-knowledge-cutover.md` |
| 2026-09-05 | Ticket 06 Library & Render Quality thin-client cutover | Manual Library sync cut over to `client.sync()` with exit-code forwarding; Library facts render probe-owned `details.paper_count` (new structured field on ready/stale library envelopes) and the maintenance orphan count; Render Quality drawer added to paper technical details via `renderAudit(key)`, new `renderReconcileStaging(key)`, per-object `promoteR`, and per-card `acceptProposal` forwarding the exact reviewed plan SHA-256; committed mutations invalidate the staging snapshot. Reconcile/render core and R/P CAS/journal/plan-hash semantics untouched. Evidence: plugin **455/455** (28 files); Python focused **193 passed**; typecheck + ruff (changed files) clean. | §2.3 Fix 54, `.scratch/thin-client/issues/06-library-and-render-quality-cutover.md` |
| 2026-09-05 | Ticket 05 corrective round 2 — noop settlement, exhaustive memory dispatcher, Stop ownership | `paper_settled(key,status)` moved from `_on_progress` to `_on_item_result`; noop branches restore counters; un-mocked real-service CLI test asserts clean NDJSON with `progress.current==1`, single `paper_settled outcome=noop`, single `item_result status=noop`, `papers_skipped==1`; `_runAllowedDispatch("memory")` is an exhaustive exact-ID chain (`embed.build`/`embed.resume`/`memory.build`/`memory.rebuild`/`memory.upgrade_backend` dispatch, unknown IDs Notice + re-probe with no substitution) with a `return` so dispatched actions never fall through to the unknown-pair probe; production-entry `it.each` regression proves legal IDs run and unknown IDs don't; `_renderMemoryDetail` Stop bound solely to `client.isOperationActive()`, legacy embed-controller fallback and stale tests deleted. Evidence: Python focused **169 passed**; plugin **448/448**; typecheck + ruff (changed files) clean. | §2.3 Fix 53, `.scratch/thin-client/issues/05-retrieval-and-knowledge-cutover.md` |
| 2026-08-21 | Publish audit from shared render-output boundary | `write_render_outputs` invokes the existing read-only `audit_paper` after `fulltext.md`/`render-map.json`; initial OCR and derived rebuild paths therefore publish the same report. Audit exceptions remain warning-only for rendering, but the boundary now replaces stale persisted report state with `FAILED` before returning. |
| 2026-08-31 | Dry-run detection boundary and remaining R/P debt (pre-closure snapshot) | Read-only checks confirmed duplicate identity/claim conflicts were blocked, but a unique existing wrong image returned `CLEAN` with zero exact repairs; semantic coverage loss was not proven by current R checks. P bounded-slot anchors still lost namespace-aware keys; fulltext duplicate identity remained report-only. Production canary was blocked on audit-FAILED handling and recovery CAS against third-party writes; these implementation gates are closed in the later 2026-08-31 entry, while semantic coverage remains open. |
...
| 2026-08-26 | D1 R promotion P0 gates and disposable canaries | Added cross-canonical `(page, block_id, bbox)` ownership, per-plan destination CAS, post-audit postcondition rollback before commit, durable promotion journal/recovery, paper-scoped writer lock, and explicit CLI scope (`OBJECT_ID` or `--all`). Four hard-kill injection points recovered; 12 disposable stratified objects promoted successfully, second promotes were no-ops, and the known page-range-invalid object stayed blocked. No production write authorized. |
| 2026-08-31 | Close reconcile P0 gates and surface safe report-only boundaries | P0-A/P0-B now fail closed on audit failures, malformed journal state/snapshots, third-party destination changes, and deterministic JSON output across Windows newlines. R legacy artifacts expose `R_CONTENT_UNVERIFIED`; bounded P anchors and fulltext duplicate/source gates are explicit. Semantic coverage remains report-only and no production rollout is authorized. | §2.18 |
| 2026-09-01 | Close remaining recovery/fulltext safety gaps | Recovery accepts only schema-v2 exact transaction namespaces, allowlisted destinations, and in-transaction backups; malformed canonical inventory is an F0 mutation block with no patches; committed report-refresh exceptions are surfaced without rollback. Disposable focused suite: 114 passed, 1 warning; production writes remain unauthorized. | §2.19 |
| 2026-09-01 | Make committed journals production-authoritative during cleanup | A committed marker means production is authoritative: recovery does not require backups, best-effort cleans the transaction directory, removes the journal, and never rolls back production. Prepared journals retain strict backup/CAS validation. |
| 2026-09-01 | Return structured status for post-commit cleanup failure | Once the journal is marked `committed`, production cannot be rolled back. Catch cleanup `OSError`, return `cleanup=pending` with the error, continue final report refresh, and let the retained committed journal be cleared by the next promotion. |
| 2026-09-02 | Harden P proposal acceptance as an authority transaction | P is the only path allowed to create a reviewed canonical object, so it must not use the old copy-then-mutate sequence. Reuse R's lock, schema-v2 journal, destination CAS, rollback, committed-cleanup recovery, provenance merge, and final audit refresh; require the reviewed snapshot and hash the exact `final-plan.json` bytes. |
...
| 2026-08-31 | **Reconcile safety closure + additional gap fixes** | Implemented and regression-covered R P0-A/P0-B fail-closed gates, report-level `R_CONTENT_UNVERIFIED`, bounded main-namespace anchors, fulltext duplicate/source blocking, malformed journal/provenance rejection, safe-ID validation, Windows-stable JSON hashing, rollback CAS, missing-page candidate blocking, and malformed fulltext/provenance-row handling. Focused suite **108 passed, 1 warning**; Ruff `E,F,I,UP` clean. No production write or front OCR/render change. | §2.18, `tests/test_promote_r.py`, `tests/test_figure_reconciliation.py`, `tests/test_fulltext_reconciliation.py` |
| 2026-08-31 | **Dry-run boundary + D1 gate sync (pre-closure snapshot)** | Initial review found the dry-run blocks duplicate canonical/ownership conflicts and missing-source cases, but missed a unique existing wrong JPG and semantic panel-coverage loss. Fresh isolated `2ZBAK9VY/figure_003` failed only its own staged-source gates. The P bounded-slot namespace, fulltext duplicate authority, R audit-FAILED gate, and recovery CAS were open at that snapshot; all except semantic coverage are closed in the later 2026-08-31 entry. | §2.17–§2.18 |
| 2026-09-01 | Reconcile follow-up safety closure | Added exact journal path/schema/destination/backup validation, malformed-inventory authority blocking with orphan regression coverage, and post-commit report-refresh failure semantics. Focused suite **114 passed, 1 warning**; Ruff and `py_compile` clean; no production write or front OCR/render change. | §2.19, `tests/test_promote_r.py`, `tests/test_fulltext_reconciliation.py` |
| 2026-09-01 | **Committed recovery cleanup closure + unverified-content census** | Closed the committed-journal backup-cleanup crash window, rejected raw destination/backup symlinks, separated report-write success from final audit `FAILED`, and ran a read-only 969-paper census: **0 affected papers / 0 affected objects / 0 audit failures**. Focused suite **116 passed, 2 skipped, 1 warning**; no production write or front OCR/render change. | §2.20, `tests/test_promote_r.py` |
| 2026-09-01 | **Post-commit cleanup status closure** | Added the cleanup-pending result contract and injection regression: a cleanup `OSError` after commit returns `ok=true, committed=true`, preserves new production bytes, leaves the committed journal, and the next promotion clears it. Focused suite **117 passed, 2 skipped, 1 warning**; no production write or front OCR/render change. | §2.20, `tests/test_promote_r.py` |
| 2026-09-02 | **P authority acceptance hardening** | Replaced copy-then-mutate proposal acceptance with a shared lock/CAS/schema-v2 journal transaction across JPG, Markdown, canonical inventory, provenance, and proposal report; exact plan snapshot/member refs are required, post-audit failures roll back before commit, committed cleanup/report failures remain recoverable, and the final audit report is refreshed. **4 new proposal safety tests; focused suite 121 passed, 2 skipped, 1 warning; no production write or front OCR/render change.** | §2.21, `tests/test_accept_proposal.py` |
| 2026-09-02 | **Exact human-reviewed plan binding** | `render reconcile --json` now exposes each staged P `final_plan_hash`; `accept-proposal KEY LABEL --plan-hash HASH` scans all safe staging candidates by exact `final-plan.json` SHA-256 instead of mtime, returning `STALE_REVIEWED_PLAN` for an unmatched token. Regression coverage includes newer unreviewed same-label plans and CLI required-argument parsing. Focused suite **126 passed, 2 skipped, 1 warning**; no production write or front OCR/render change. | §2.22, `tests/test_accept_proposal.py`, `tests/test_figure_reconciliation.py` |
| 2026-09-02 | **Stale render-consistency report invalidation** | Added atomic fail-closed replacement of a prior render-consistency report when the automatic audit raises or returns a non-object; lineage now projects `FAILED` (or `NOT_RUN` after best-effort cleanup) instead of old `CLEAN`/`DEGRADED` state. Regression suite **132 passed, 2 skipped, 1 warning**; no production write or front OCR/render corpus change. | §2.23, `tests/test_ocr_fulltext_state.py` |
| 2026-09-02 | **Reconcile Core Frozen & Contract Mapping** | Established `docs/RECONCILE-EXTENSION-CONTRACT.md` (8 extension rules, minimal finding schema, fail-closed R/P invariants) and `docs/BACKEND-FRONTEND-CONTRACT-MAPPING.md` (5 backend contracts, 6 product projection domains, PaperForgeClient boundary). Reconcile mutation engine is frozen; active work shifts to backend surface rationalization. | §2.23, §4 |
| 2026-09-03 | **Ticket 04 — OCR Workspace & Processing Domain Cutover** | OCR Workspace observation, actions, progress, and cancellation now cross the `PaperForgeClient` boundary; direct Workspace runtime/subprocess/controller seams and the stale `_resolvePython()` tests were removed. Focused plugin suite **60 passed / 0 failed**; typecheck clean. | §2.24, §4 |
