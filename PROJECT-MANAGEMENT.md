...
>
> **Active work:** Reconcile-only R promotion is **disposable-canary PASS but production-canary BLOCKED**. R P0-A/P0-B fail-closed audit, recovery-CAS, exact journal-path, and report-refresh handling are implemented and regression-covered. Fulltext malformed-authority mutation is blocked; semantic coverage remains report-only and production rollout still requires owner authorization.
>
> ---
>
> **Current state:** The front OCR/render pipeline remains unchanged. The independent reconcile layer detects and blocks duplicate canonical identities, cross-canonical claims, ambiguous provenance, malformed recovery journals, unsafe promotion IDs, and malformed fulltext authority; it stages selected R plans in isolation and protects explicit promotion with manifest/CAS/journal gates. A committed promotion now reports post-commit audit-refresh failure explicitly without rolling back committed artifacts. No production materialization or canonical inventory mutation was performed.
