...
| Reconcile P authority acceptance (2026-09-02) | **121 passed, 2 skipped, 1 warning** across proposal acceptance, R promotion, render audit, figure reconciliation, fulltext authority, and lineage; targeted Ruff `E,F,I,UP`, `py_compile`, and `git diff --check` clean. Added shared writer locking, strict reviewed-plan snapshots, true final-plan SHA-256 provenance, five-target journaled atomic writes, CAS rollback, committed cleanup recovery, and final audit/report refresh. ✅ (disposable fixtures only) |
...
`paperforge render accept-proposal KEY LABEL` now uses the same paper-scoped writer lock as R promotion. It fails closed on missing/stale reviewed snapshots, unsafe paths, malformed plans, invalid staged images/Markdown, duplicate identity or asset claims, and malformed canonical authority. The exact `member_refs` from the reviewed plan are preserved; no rematching is performed.

The action prepares one schema-v2 journal for the canonical JPG, Markdown, `figure_inventory.json`, materialization provenance, and proposal report; writes each target atomically; verifies the post-write audit before the committed marker; and restores all snapshots on failure. The committed marker makes production authoritative, so cleanup/report-refresh failures are surfaced without rollback. `accepted_from.final_plan_hash` and provenance now store the direct SHA-256 of `final-plan.json`, and the final consistency report is refreshed after commit.
