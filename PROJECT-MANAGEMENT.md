...
>
> **Ticket 04 (2026-09-03):** OCR Workspace and processing now use `PaperForgeClient` observation/action contracts; no Workspace runtime resolver, `execFile`, or `OcrProcessController` path remains.
>
> ---
>
...
| OCR Workspace & Processing Domain Cutover (2026-09-03) | **60 plugin tests passed / 0 failed; `npm run typecheck` clean**. MockTransport covers lineage/table loading, global activity, action availability, NDJSON phase/item progress, canonical Redo dispatch, and client-owned cancellation. ✅ |
...
| 51 | — | OCR Workspace read/action path bypassed the thin-client boundary | Thin-client cutover | Table rows now use `probe("lineage")` + `queryOcrPapers()`; Run OCR, Redo, and Rebuild Derived use descriptor-gated `runAction()`; progress and cancellation use the client operation lock; stale runtime/controller seams were removed. | — |
...
### 2.24 OCR Workspace and Processing Domain Cutover (2026-09-03 — COMPLETE)

The OCR Workspace now derives paper rows from `PaperForgeClient.probe("lineage")` and the typed `queryOcrPapers()` result. Backend lineage controls stale/update, incomplete, failed, queued, and processing projections; the OCR probe supplies global activity truth. Run OCR and user-facing Redo map to canonical `ocr.run`, while Rebuild Derived maps to `ocr.rebuild_derived`; descriptors gate availability and confirmation before the client builds the scoped action request. NDJSON phase, progress, item-result, terminal, and cancellation events drive the activity panel. Stop is enabled only while this client owns the active operation handle.

The Workspace no longer resolves Python runtimes or imports `execFile`/`OcrProcessController`; `ocr.redo` remains an internal legacy command outside the production Workspace path. MockTransport regressions cover the client boundary, action scope, busy descriptors, progress, refresh, and cooperative cancellation.

Verification: **60 plugin tests passed / 0 failed** across the focused client/OCR suite; `npm run typecheck` clean. No OCR/render materialization was run.
...
0.3. ✅ **Ticket 04 — OCR Workspace & Processing Domain Cutover** — thin-client observation/action migration complete; focused MockTransport regressions and typecheck pass. `ocr.redo` stays internal-only; user-facing Redo uses canonical `ocr.run`.
...
| 2026-09-03 | Ticket 04 OCR Workspace thin-client cutover | Moved OCR table/read state to lineage + typed client queries; routed Run OCR, Redo, and Rebuild Derived through descriptor-gated `runAction`; streamed phase/item progress and client-owned cancellation; replaced stale runtime/controller tests with MockTransport coverage. Focused suite **60 passed / 0 failed**; typecheck clean. | §2.24, `.scratch/thin-client/issues/04-ocr-workspace-cutover.md` |
