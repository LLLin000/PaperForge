...
def export_collection_paths(export_path: Path) -> list[str]:
    ...
    paths = sorted(
        {
            path
            for path in lookup.get("path_by_key", {}).values()
...
def load_domain_collection_catalog(paths: dict[str, Path]) -> dict[str, list[str]]:
    ...
    entry_by_export = {
        entry.get("export_file", ""): dict(entry)
        for entry in domain_entries
        if entry.get("export_file")
    }
    ...
    for export_path in export_files:
        ...
        if entry.get("allowed_collections", []) != derived:
            ...
            changed = True
    domains = sorted(
        entry_by_export.values(), key=lambda entry: entry.get("domain", "")
    )
    if changed or config.get("domains", []) != domains:
        write_json(config_path, {"domains": domains})
    return {
        entry.get("domain", ""): entry.get("allowed_collections", [])
...
def resolve_collection_choice(domain: str, raw_value: str, catalog: dict[str, list[str]]) -> dict[str, str]:
    ...
    if "/" not in text:
        leaf_matches = [
            path
            for path in allowed
            if path.split("/")[-1].strip().lower() == lower_text
        ]
    ...
    suffix_matches = [
        path
        for path in allowed
        if path.lower().endswith("/" + lower_text) or path.lower() == lower_text
    ]
    ...
    compact_matches = []
    for path in allowed:
        path_compact = re.sub(r"\s+", "", path.lower())
...
def apply_candidate_collection_resolution(row: dict, catalog: dict[str, list[str]]) -> dict:
    ...
    recommended = resolve_collection_choice(
        domain, resolved.get("recommended_collection", ""), catalog
    )
...
def resolve_item_collection_paths(item: dict, collection_lookup: dict) -> list[str]:
    ...
    return sorted(
        {path for path in paths if path}, key=lambda value: (-value.count("/"), value)
    )
...
def collection_fields(collection_paths: list[str]) -> dict[str, str | list[str]]:
    ...
    if paths:
        primary = sorted(
            paths, key=lambda value: (value.count("/"), len(value), value), reverse=True
        )[0]
    tags = []
    seen = set()
    for path in paths:
    ...
    return {
        ...
        "collection_group": [group] if group else [],
    }
    ...
    if isinstance(data, dict) and isinstance(data.get("items"), list):
        ...
        for item in data["items"]:
            ...
            for attachment in item.get("attachments", []):
                ...
                content_type = (
                    "application/pdf"
                    if str(attachment_path).lower().endswith(".pdf")
                    else ""
                )
                attachments.append(
                    {"path": attachment_path, "contentType": content_type}
                )
            rows.append(
                {
                    ...
                    "doi": item.get("DOI", ""),
                    "pmid": item.get("PMID", ""),
                    "collections": resolve_item_collection_paths(
                        item, collection_lookup
                    ),
...
def extract_preserved_deep_reading(text: str) -> str:
    ...
    if not text:
        return ""
    match = re.search(r'^## 🔍 精读\s*$', text, re.MULTILINE)
    if not match:
        return ""
    start = match.start()
...
def has_deep_reading_content(text: str) -> bool:
    ...
    for line in lines:
        ...
        if stripped.startswith("### "):
            continue
        # Skip callout headers (e.g. `> [!note] 图像定位`, `> [!abstract] 一句话总览`)
        if re.match(r'^>\s*\[!', stripped):
            continue
        # Skip lines containing placeholder text
        if "（待补充）" in stripped:
            continue
        # Skip empty list items (`- ` or `* `)
        if re.match(r'^[-*]\s*$', stripped):
            continue
        non_placeholder_chars += len(stripped)
        # A "prose sentence" contains CJK characters and ends with punctuation
        if re.search(r'[\u4e00-\u9fff]', stripped) and re.search(r'[。！？\.\!\?]$', stripped):
...
def parse_existing_library_record(path: Path) -> dict:
    ...
    for key in ("analyze", "recommend_analyze", "do_ocr"):
        match = re.search(rf"^{key}:\s*(true|false)$", text, re.MULTILINE)
        if match:
...
def load_control_actions(paths: dict[str, Path]) -> dict[str, dict]:
    ...
    for record in paths["library_records"].rglob("*.md"):
        ...
        actions[zotero_key] = {
            "analyze": row.get("analyze", False),
            "do_ocr": row.get("do_ocr", False),
        }
    return actions
...
def run_selection_sync(vault: Path) -> int:
    ...
    for export_path in sorted(paths["exports"].glob("*.json")):
        ...
        for item in load_export_rows(export_path):
            ...
            validated_ocr_status, validated_error = (
                validate_ocr_meta(paths, meta) if meta else ("pending", "")
            )
            if meta:
            ...
            note_path = (
                ...
                / "Literature"
                / domain
                / f"{item['key']} - {slugify_filename(item['title'])}.md"
            )
            ...
            content = library_record_markdown(
                {
                    ...
                    "pdf_path": obsidian_wikilink_for_pdf(
                        vault, pdf_attachments[0]["path"]
                    )
                    if pdf_attachments
                    else "",
                    ...
                    if note_text and has_deep_reading_content(note_text)
                    else "pending",
...
def writeback_command_for_candidate(row: dict) -> dict | None:
    ...
    if existing_zotero_key:
        command.update(
            {
                "action": "attach_existing_item_to_collection",
                "existing_zotero_key": existing_zotero_key,
            }
        )
        return command
...
def sync_writeback_queue(paths: dict[str, Path], candidate_map: dict[str, dict]) -> tuple[list[dict], int]:
    ...
    for candidate_id, row in candidate_map.items():
        ...
        if existing and str(existing.get("status", "") or "").strip() in {
            "queued",
            "running",
            "processed",
        }:
...
def candidate_id_from_payload(row: dict) -> str:
    ...
    doi = re.sub(r"[^a-z0-9]+", "-", str(row.get("doi", "") or "").strip().lower()).strip("-")
    pmid = re.sub(r"[^0-9]+", "", str(row.get("pmid", "") or "").strip())
    fallback = re.sub(r"[^a-z0-9]+", "-", normalize_candidate_title(row.get("title", ""))).strip("-")[:80]
...
def adapt_google_scholar_candidate(row: dict) -> dict:
    payload = dict(row.get("payload") or {})
    adapted = {
        "candidate_id": row.get("candidate_id") or f"google-scholar-{re.sub(r'[^a-z0-9]+', '-', normalize_candidate_title(payload.get('title', ''))).strip('-')[:80]}",
        "domain": row.get("domain", ""),
        "title": payload.get("title", ""),
...
def search_arxiv(task: dict, limit: int) -> tuple[list[dict], dict]:
    ...
    url = (
        ...
        f"?search_query={encoded_query}&start=0&max_results={limit}&sortBy=relevance&sortOrder=descending"
    )
    headers = {"User-Agent": os.environ.get("LIT_PIPELINE_USER_AGENT", default_user_agent())}
        ...
        entries.append(
            {
                ...
                "authors": [{"name": (node.findtext("atom:name", default="", namespaces=ns) or "").strip()} for node in entry.findall("atom:author", ns)],
...
def normalize_candidate_payload(row: dict) -> dict:
    normalized = {
        ...
        for key, value in adapt_candidate_event(row).items()
    }
    ...
    normalized["source"] = (
        str(normalized.get("source", "") or "").strip() or "candidate_ingest"
    )
    normalized["candidate_source_type"] = (
        str(normalized.get("candidate_source_type", "") or "").strip()
        or normalized["source"]
    )
    normalized["decision"] = canonicalize_decision(normalized.get("decision", ""))
    normalized["import_status"] = (
        str(normalized.get("import_status", "") or "").strip() or "pending"
    )
    normalized["recommend_confidence"] = (
        str(normalized.get("recommend_confidence", "") or "").strip() or "0"
    )
...
def _normalize_harvest_row(row: dict[str, str]) -> dict:
    normalized = normalize_candidate_payload(
        {key: _normalize_harvest_value(value) for key, value in row.items()}
    )
    normalized["source"] = normalized.get("source", "") or "reference_harvest"
    normalized["candidate_source_type"] = (
        normalized.get("candidate_source_type", "") or "reference_harvest"
    )
    return normalized
...
def frontmatter_note(entry: dict, existing_text: str = "") -> str:
    ...
    lines.extend(
        [
            ...
            "## 💡 文献内容总结",
            "",
            "- 由 index-refresh worker 自动生成的正式文献卡片。",
            "- 精读笔记（Deep Reading）仅由 /LD-deep 命令维护；index-refresh 只保留已有内容，不自动生成。",
            "- 如需精读，请在 Base 中勾选 analyze，OCR 完成后运行 /LD-deep <zotero_key>。",
            "",
        ]
    )
    if preserved_deep:
...
def analyze_selected_keys(paths: dict[str, Path]) -> set[str]:
    return {
        key for key, row in load_control_actions(paths).items() if row.get("analyze")
    }
...
def run_index_refresh(vault: Path) -> int:
    ...
    # Load all exports for sync logic
    exports = {}
    for export_path in sorted(paths["exports"].glob("*.json")):
        domain = domain_lookup.get(export_path.name, export_path.stem)
    ...
    # selected_keys = analyze_selected_keys(paths)  # Disabled: index all exports
    selected_keys = None  # None means process all items
    index_rows = []
    ...
    for export_path in sorted(paths["exports"].glob("*.json")):
        ...
        for item in export_rows:
            ...
            entry = {
                ...
                "collection_group": collection_meta.get("collection_group", []),
                "has_pdf": bool(pdf_attachments),
                "pdf_path": obsidian_wikilink_for_pdf(vault, pdf_attachments[0]["path"])
                if pdf_attachments
                else "",
                "ocr_status": meta.get("ocr_status", "pending"),
                "ocr_job_id": meta.get("ocr_job_id", ""),
                "ocr_md_path": obsidian_wikilink_for_path(
                    vault, meta.get("markdown_path", "")
                ),
                ...
                if note_path.exists()
                and has_deep_reading_content(note_path.read_text(encoding="utf-8"))
                else "",
            }
    ...
    # === Sync library-records: remove orphaned/duplicate records ===
    ...
    if control_records_dir.exists():
        for domain_dir in control_records_dir.iterdir():
            ...
            # Find duplicates to delete (orphaned + duplicate + no PDF)
            to_delete = []
            for normalized, keys in records_by_title.items():
                ...
                if keys_in_export and keys_not_in_export:
                    # Keep one with PDF if available
            ...
            if deleted_count > 0:
                print(
                    f"index-refresh: cleaned {deleted_count} orphaned records in {domain}"
                )
...
def validate_ocr_meta(paths: dict[str, Path], meta: dict) -> tuple[str, str]:
    ...
    if rendered_pages != page_count:
        return (
            "done_incomplete",
            f"OCR page marker mismatch: meta={page_count}, rendered={rendered_pages}",
        )
    return "done", ""
...
def normalize_obsidian_markdown(text: str) -> str:
    ...
    normalized = re.sub(
        r"\$\^\{([^}]*)\\dagger\}\$",
        lambda m: _superscript_to_equal_footnote(m.group(1)),
        normalized,
    )
    normalized = re.sub(
        r"\$\^\{([^}]*)†\}\$",
    ...
    normalized = re.sub(
        r"(?m)^\*\s*Correspondence:\s*(.+)$",
        lambda match: f"[^correspondence]: {match.group(1).strip()}",
        normalized,
    )
    normalized = re.sub(
        r"(?m)^Correspondence:\s*(.+)$",
        lambda match: f"[^correspondence]: {match.group(1).strip()}",
        normalized,
    )
    normalized = re.sub(
        r"(?m)^†\s*(.+)$",
        lambda match: f"[^equal]: {match.group(1).strip()}",
        normalized,
    )
...
def _superscript_to_equal_footnote(content: str) -> str:
    cleaned = re.sub(r"(,)?\\dagger", "", content).replace("†", "").strip(", ")
    if cleaned:
...
def is_subfigure_label(text: str) -> bool:
    ...
    return bool(
        re.fullmatch(r"(?:\([a-z]\)\s*)+", compact)
        or re.fullmatch(r"[a-z]", compact)
        or re.fullmatch(r"[a-z]\)", compact)
    )
...
def is_frontmatter_noise_line(text: str) -> bool:
    ...
    if lower.startswith(prefixes):
        return True
    if re.fullmatch(r"\d+\s+\w{1,6}", compact):
        return True
...
def is_reference_tail_noise_line(text: str) -> bool:
    ...
    patterns = (
        ...
        r"^Submit your next manuscript",
        r"^BioMed Central\b",
        r"^Convenient online submission$",
        r"^Thorough peer review$",
        r"^No space constraints",
        r"^Immediate publication on acceptance$",
        r"^Inclusion in PubMed, CAS, Scopus and Google Scholar$",
        r"^Research which is freely available for redistribution$",
        r"^[•\-]\s*(Convenient online submission|Thorough peer review|No space constraints|Immediate publication on acceptance|Inclusion in PubMed|Research which is freely available)",
    )
...
def parse_reference_number(text: str) -> int | None:
    match = re.match(r"^\s*(\d+)\.", clean_block_text(text))
    return int(match.group(1)) if match else None


def sort_reference_blocks(blocks: list[dict]) -> list[dict]:
    numbered_count = sum(
        parse_reference_number(block.get("block_content", "")) is not None
        for block in blocks
    )
    ...
    return sorted(
        blocks,
        key=lambda block: (
            block.get("block_bbox", [0, 0, 0, 0])[0],
            block.get("block_bbox", [0, 0, 0, 0])[1],
        ),
    )


def assign_reference_continuation(
    continuation_block: dict, reference_blocks: list[dict]
) -> int | None:
    ...
    for index, ref_block in enumerate(reference_blocks):
        ...
        same_column = (
            horizontal_overlap > 0 or abs(((x1 + x2) / 2) - ((rx1 + rx2) / 2)) < 180
        )
...
def handle_first_page_metadata_lines(
    ...
    for line in lines:
        ...
        if line.startswith("Correspondence:") or line.startswith("* Correspondence:"):
            footnotes[:] = [
                fn for fn in footnotes if not fn.startswith("[^correspondence]:")
            ]
        ...
        if "$^{" in line and "," in line and " and " in line:
            ...
            handled = True
            continue
        if len(line) < 22 or re.fullmatch(r"\d+\s+\w+", line):
            handled = True
            continue
    return handled
...
def parse_vision_footnote_entries(content: str) -> list[tuple[str, str]]:
    ...
    if not cleaned:
        return []
    matches = re.findall(r"\$?\^\{?([A-Za-z*]+)\}?\$?\s*([^;]+)", cleaned)
    if matches:
        return [
            (marker.strip(), body.strip()) for marker, body in matches if body.strip()
        ]
...
def _parse_asset_bbox_from_line(line: str) -> tuple[int, int, int, int] | None:
    match = re.search(r"page_(\d+)_(?:figure|table)_(\d+)_(\d+)_(\d+)_(\d+)\.jpg", line)
    if not match:
        return None
...
def _bbox_contains(
    outer: tuple[int, int, int, int], inner: tuple[int, int, int, int], margin: int = 12
) -> bool:
    ...
    for idx, bbox in media_indexes:
        for other_idx, other_bbox in media_indexes:
            if idx == other_idx:
                continue
            if (
                _bbox_contains(other_bbox, bbox)
                and _bbox_area(other_bbox) > _bbox_area(bbox) * 1.15
            ):
...
def attach_table_footnotes(table_html: str, marker_to_id: dict[str, str]) -> str:
    ...
    return re.sub(r"([a-z](?:\s*,\s*[a-z])*)</td>", replacer, table_html)
...
def is_formal_figure_legend(text: str) -> bool:
    ...
    return bool(
        re.match(
            r"^(?:"
            r"Extended\s+Data\s+Fig\.?\s+\w+|"
            r"Extended\s+Data\s+Figure\s+\w+|"
            r"Extended\s+Data\s+Table\s+\w+|"
            r"Supplementary\s+Fig\.?\s+\w+|"
            r"Supplementary\s+Figure\s+\w+|"
            r"Supplementary\s+Table\s+\w+|"
            r"Supplementary\s+Video\s+\w+|"
            r"Figure\s+\d+|"
            r"Fig\.?\s+\d+|"
            r"Table\s+\d+|"
            r"Scheme\s+\w+|"
            r"Graphical\s+Abstract(?:\s*[:|.\-].*)?"
            r")",
            cleaned,
            flags=re.IGNORECASE,
        )
...
def estimate_body_column_width(blocks: list[dict], page_width: int = 0) -> int:
    ...
    for block in blocks:
        ...
        if (
            ...
            or re.match(r"^(?:Figure|Fig\.?|Table)\s+\w+", text, flags=re.IGNORECASE)
        ):
            continue
...
def _precaption_media_region(caption_bbox: list[int], cluster_bboxes: list[list[int]]) -> list[int] | None:
    relevant = [
        ...
        if int(bbox[3]) <= int(caption_bbox[1]) + 24
    ]
    if len(relevant) < 1:
        return None
    return [
...
def compute_precaption_composite_regions(
    blocks: list[dict], page_width: int = 0, page_height: int = 0
) -> list[dict]:
    ...
    for caption in caption_blocks:
        ...
        for block in blocks:
            ...
            near_region_side = (
                vertical_overlap_with_region > 0
                and (
                    0 <= precaption_region[0] - bbox[2] <= 80
                    or 0 <= bbox[0] - precaption_region[2] <= 80
                )
            )
            intersects_region = (
            ...
            if label in {"text", "paragraph_title"}:
                ...
                if (
                    text
                    and not re.match(
                        r"^(?:Extended\s+Data\s+Fig\.?|Extended\s+Data\s+Figure|Figure|Fig\.?|Table)\s+\w+",
                        text,
                        flags=re.IGNORECASE,
                    )
                    and (
                        width <= int(max(body_column_width, 1) * 0.78)
                        or is_embedded_figure_text_block(
                            block, blocks, page_width=page_width, page_height=page_height
                        )
                    )
                ):
                    region_blocks.append(block)
        media_ids = {
            ...
            if block.get("block_label") in {"image", "chart"}
        }
        text_ids = {
            block.get("block_id")
            for block in region_blocks
            if block.get("block_label") in {"text", "paragraph_title"}
        }
...
def is_embedded_figure_text_block(
    ...
    for cluster_bbox in cluster_bboxes:
        ...
        if (
            ...
            center_inside_x
            and abs(_bbox_center_y(bbox) - _bbox_center_y(cluster_bbox)) <= max(80, height * 4)
        ):
            close_media_count += 1
        ...
        if vertical_overlap > 0 and (
            0 <= int(bbox[0]) - int(cluster_bbox[2]) <= 60
            or 0 <= int(cluster_bbox[0]) - int(bbox[2]) <= 60
        ):
            side_media = True
        dx = 0
...
def is_embedded_vision_footnote_block(
    ...
    if _bbox_width(bbox) <= 0 or _bbox_height(bbox) <= 0:
        return False
    composite_regions = compute_precaption_composite_regions(
        blocks, page_width=page_width, page_height=page_height
    )
    ...
    for cluster in clusters:
        ...
        if _bbox_contains(tuple(cluster_bbox), tuple(bbox), margin=24):
            return True
        if _bbox_horizontal_overlap(bbox, cluster_bbox) > 0 and _bbox_vertical_overlap(
            bbox, cluster_bbox
        ) > 0:
            return True
        vertical_overlap = _bbox_vertical_overlap(bbox, cluster_bbox)
        near_side = (
            ...
            and (
                0 <= int(cluster_bbox[0]) - int(bbox[2]) <= 60
                or 0 <= int(bbox[0]) - int(cluster_bbox[2]) <= 60
            )
        )
        if near_side:
            return True
...
def caption_group_assignments(
    ...
    for block in blocks:
        ...
        elif re.match(r"^(?:Table|Extended\s+Data\s+Table|Supplementary\s+Table)\s+\d+", text, flags=re.IGNORECASE):
...
def render_page_blocks(
    ...
    raw_reference_blocks = [
        block for block in blocks if block.get("block_label") == "reference_content"
    ]
    first_reference_y = min(
        (
            block.get("block_bbox", [0, 10**9, 0, 0])[1]
            for block in raw_reference_blocks
        ),
        default=10**9,
    )
    ...
    page_image = render_pdf_page_cached(
        ...
        ocr_width,
        ocr_height,
        page_cache_dir / f"page_{page_index:03d}.png",
    )
    if not page_image:
    ...
    composite_regions = compute_precaption_composite_regions(
        blocks, page_width=ocr_width, page_height=ocr_height
    )
    ...
    caption_linked_media_ids = {
        item.get("block_id")
        for media_list in list(figure_caption_map.values())
        + list(table_caption_map.values())
    ...
    for block in blocks:
        ...
        if composite_region:
            if not composite_region.get("rendered"):
                ...
                if page_image and crop_block_asset(page_image, region_bbox, asset_path):
                    rendered.append(
                        _image_embed_for_obsidian(asset_path.relative_to(vault))
                    )
            continue
        ...
        if label == "paragraph_title":
            ...
            if (
                ...
                or is_embedded_figure_text_block(
                    block, blocks, page_width=ocr_width, page_height=ocr_height
                )
            ):
                continue
        ...
        if label == "text":
            ...
            if page_index == 1 and not first_page_meta_done:
                if handle_first_page_metadata_lines(
                    text, rendered, affiliation_buffer, deferred_meta, footnotes
                ):
                    continue
        ...
        if label == "figure_title":
            ...
            if not is_formal_figure_legend(caption_text):
                continue
            linked_media = figure_caption_map.get(
                block.get("block_id"), []
            ) or table_caption_map.get(block.get("block_id"), [])
            if linked_media and page_image:
                rendered_caption_media_ids.update(
                    item.get("block_id") for item in linked_media
                )
                union_bbox = [
                ...
                asset_kind = (
                    "figure" if block.get("block_id") in figure_caption_map else "table"
                )
                asset_path = (
                    images_dir
                    / "blocks"
                ...
                if asset_kind == "table":
                    for item in linked_media:
                        if item.get("block_label") == "table" and item.get(
                            "block_content"
                        ):
                            rendered.append(
                                clean_block_text(item.get("block_content", ""))
                            )
                            break
        ...
        if label == "vision_footnote":
            if is_formal_figure_legend(content):
                rendered.append(clean_block_text(content))
                continue
            if is_embedded_vision_footnote_block(
                block, blocks, page_width=ocr_width, page_height=ocr_height
            ):
                continue
            ...
            for marker, body in entries:
                ...
                footnotes.append(
                    f"[^{footnote_id}]: {body or clean_block_text(content)}"
                )
            if rendered and marker_to_id:
                ...
                if last.startswith("<table>"):
                    ...
                    for marker, footnote_id in marker_to_id.items():
                        rendered[-1] = attach_footnote_reference(
                            rendered[-1], marker, footnote_id
                        )
            continue
    if footnotes:
        rendered.append("")
    ...
    if reference_blocks:
        ...
        for index, continuation in zip(
            incomplete_reference_indexes, sorted_continuations
        ):
        ...
        for continuation in sorted_continuations:
            if id(continuation) in assigned_continuation_indexes:
                continue
            target_index = assign_reference_continuation(
...
def postprocess_ocr_result(
    vault: Path, key: str, all_results: list[dict]
    ...
    markdown_path = (
        str(fulltext_path.relative_to(vault)).replace("\\", "/") if page_num else ""
    )
...
def run_ocr(vault: Path) -> int:
    ...
    target_keys = {
        key for key, action in control_actions.items() if action.get("do_ocr", False)
    }
    target_rows = []
    for export_path in sorted(paths["exports"].glob("*.json")):
        for item in load_export_rows(export_path):
    ...
    if not token:
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Environment") as env_key:
                token = str(
                    winreg.QueryValueEx(env_key, "PADDLEOCR_API_TOKEN")[0]
                ).strip()
        except Exception:
            token = ""
    ...
    for queue_row in ocr_queue:
        ...
        if status in {"queued", "running"} and meta.get("ocr_job_id"):
            ...
            if state in {"pending", "running"}:
                ...
                page_num, markdown_path, json_path, fulltext_md_path = (
                    postprocess_ocr_result(vault, key, all_results)
                )
    ...
    if queue_changed:
        ocr_queue = [
            ...
            if str(row.get("queue_status", "")).lower() != "done"
        ]
...
def run_deep_reading(vault: Path) -> int:
    ...
    for export_path in sorted(paths["exports"].glob("*.json")):
        ...
        for item in load_export_rows(export_path):
            ...
            # Check library record for analyze flag
            ...
            # Check formal note for deep reading content
            ...
            # Build pending queue (analyze=true + pending)
    ...
    if pending_queue:
        ...
        report_lines.extend([
            "## 操作",
            "",
            "- 对就绪论文，使用 `/LD-deep <zotero_key>` 触发精读",
...
def main() -> int:
    ...
    if args.worker == "candidate-sync":
        return run_candidate_sync(args.vault)
    ...
    if args.worker == "search-sources":
        return run_search_sources(args.vault)
    if args.worker == "ingest-candidates":
        return run_ingest_candidates(args.vault)
    if args.worker == "harvest-sync":
        return run_harvest_sync(args.vault)
    if args.worker == "selection-sync":
        return run_selection_sync(args.vault)
    if args.worker == "prepare-writeback":
        return run_prepare_writeback(args.vault)
    if args.worker == "writeback":
        return run_writeback(args.vault)
    if args.worker == "writeback-native":
        return run_writeback_native(args.vault)
