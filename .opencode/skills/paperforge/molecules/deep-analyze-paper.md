...
> 不重复执行 `atoms/retrieval-routing.md` §6 的通用 Object evidence 协议。

如果尚未有唯一 zotero_key，先定位：打开 `molecules/pre-read.md`
Step 1(Locate)获取 KEY。多候选则让用户选择。
获得唯一 KEY 后进入 Step 0。
...
- `verified: true` → 可作为复查优先级；仍必须回到原文回答事实问题。
